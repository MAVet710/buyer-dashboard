...
import_replacement = import_anchor + nl.join([
...
if import_anchor not in text:
    raise SystemExit("buyer intelligence import anchor not found in main app.py")
text = text.replace(import_anchor, import_replacement, 1)

start = text.index("def ai_inventory_check(detail_view, doh_threshold, data_source):")
end = text.index(nl + "def _feature_enabled(", start)
new_function = nl.join([
...
if old_ui not in text:
    raise SystemExit("legacy inventory AI UI block not found in main app.py")
