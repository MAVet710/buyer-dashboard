...
OPEN_ORDER_STATUSES = {"draft", "confirmed", "allocated", "partially_fulfilled"}
ALLOCATABLE_SALES_STATUSES = {"confirmed", "allocated", "partially_fulfilled"}
...
class CommercialRepository:
    ...
    def create_trade_partner(self, organization_id: str, *, name: str, partner_type: str, actor: str, **details: Any) -> TradePartner:
        ...
        with self._session_factory.begin() as session:
            ...
            session.add(partner)
            session.flush()
            self._audit(session, organization_id, None, "trade_partner", partner.id, "created", actor, {"partner_type": normalized_type, "name": clean_name})
        return partner

    def list_trade_partners(self, organization_id: str, *, active_only: bool = True) -> list[TradePartner]:
        with self._session_factory() as session:
            statement = select(TradePartner).where(TradePartner.organization_id == organization_id)
            if active_only:
        ...
        *,
        ...
        with self._session_factory.begin() as session:
            ...
            if partner.partner_type not in {required_partner_type, "both"}:
                raise ValueError(f"{normalized_type.title()} orders require a {required_partner_type} partner.")
            ...
            for position, raw in enumerate(lines, start=1):
                ...
                session.add(CommercialOrderLine(
                    organization_id=organization_id,
    ...
    def list_orders(self, organization_id: str, facility_id: str, *, open_only: bool = False) -> list[CommercialOrder]:
        with self._session_factory() as session:
            statement = select(CommercialOrder).where(CommercialOrder.organization_id == organization_id, CommercialOrder.facility_id == facility_id)
            if open_only:
                statement = statement.where(CommercialOrder.status.in_(OPEN_ORDER_STATUSES))
            return list(session.scalars(statement.order_by(CommercialOrder.due_at.is_(None), CommercialOrder.due_at, CommercialOrder.created_at.desc())))

    def list_order_lines(self, organization_id: str, *, order_id: str | None = None) -> list[CommercialOrderLine]:
        with self._session_factory() as session:
            statement = select(CommercialOrderLine).where(CommercialOrderLine.organization_id == organization_id)
            if order_id:
                statement = statement.where(CommercialOrderLine.commercial_order_id == order_id)
            return list(session.scalars(statement.order_by(CommercialOrderLine.commercial_order_id, CommercialOrderLine.position)))

    def list_allocations(self, organization_id: str, facility_id: str, *, order_id: str | None = None) -> list[OrderLotAllocation]:
        with self._session_factory() as session:
            statement = select(OrderLotAllocation).where(OrderLotAllocation.organization_id == organization_id, OrderLotAllocation.facility_id == facility_id)
            if order_id:
                statement = statement.where(OrderLotAllocation.commercial_order_id == order_id)
            return list(session.scalars(statement.order_by(OrderLotAllocation.created_at)))

    def confirm_order(self, order_id: str, *, organization_id: str, facility_id: str, actor: str) -> CommercialOrder:
        return self._update_order_status(order_id, organization_id=organization_id, facility_id=facility_id, status="confirmed", actor=actor)

    def cancel_order(self, order_id: str, *, organization_id: str, facility_id: str, actor: str) -> CommercialOrder:
        with self._session_factory.begin() as session:
            order = self._require_order(session, order_id, organization_id, facility_id)
            if order.status == "fulfilled":
                raise ValueError("A fulfilled order cannot be cancelled.")
            order.status = "cancelled"
            order.updated_by = actor
            allocations = session.scalars(select(OrderLotAllocation).where(OrderLotAllocation.commercial_order_id == order.id, OrderLotAllocation.status.in_(["reserved", "partial"])))
            for allocation in allocations:
                allocation.status = "released"
            self._audit(session, organization_id, facility_id, "commercial_order", order.id, "cancelled", actor, {})
            return order

    def set_payment_status(self, order_id: str, *, organization_id: str, facility_id: str, payment_status: str, actor: str) -> CommercialOrder:
        ...
        with self._session_factory.begin() as session:
            ...
            order.updated_by = actor
            self._audit(session, organization_id, facility_id, "commercial_order", order.id, "payment_status_changed", actor, {"from": previous, "to": normalized})
            return order

    def allocate_lot(self, *, organization_id: str, facility_id: str, order_line_id: str, lot_id: str, quantity: float, actor: str) -> OrderLotAllocation:
        ...
        with self._session_factory.begin() as session:
            ...
            if order.status not in ALLOCATABLE_SALES_STATUSES:
                raise ValueError("Confirm the sales order before allocating inventory.")
            lot = session.get(InventoryLot, lot_id)
            if not lot or lot.organization_id != organization_id or lot.facility_id != facility_id or lot.product_id != line.product_id or lot.status not in {"available", "released"}:
            ...
            lot_availability = availability["by_lot"].get(lot.id)
            if not lot_availability:
                raise ValueError("The lot is not available in the organization inventory projection.")
            hard_free = max(
                0.0,
                float(lot_availability["on_hand"])
                - float(lot_availability["production_reserved"])
                - float(lot_availability["wholesale_reserved"]),
            )
            if requested > hard_free + 1e-9:
                raise ValueError("Allocation exceeds unreserved lot inventory after Production and Wholesale claims.")

            line_allocated = float(session.scalar(select(func.coalesce(func.sum(OrderLotAllocation.quantity), 0.0)).where(
            ...
            allocation = session.scalar(select(OrderLotAllocation).where(OrderLotAllocation.commercial_order_line_id == line.id, OrderLotAllocation.lot_id == lot.id))
            ...
            order.updated_by = actor
            session.flush()
            self._audit(session, organization_id, facility_id, "commercial_order", order.id, "lot_allocated", actor, {"line_id": line.id, "lot_id": lot.id, "quantity": requested})
            return allocation

    def post_fulfillment(self, *, organization_id: str, facility_id: str, order_line_id: str, lot_id: str, quantity: float, actor: str, reference: str = "") -> InventoryTransaction:
        ...
        with self._session_factory.begin() as session:
            ...
            allocation = None
            if order.order_type == "sales":
                allocation = session.scalar(select(OrderLotAllocation).where(
                    OrderLotAllocation.commercial_order_line_id == line.id,
    ...
    def receive_purchase_line(self, *, organization_id: str, facility_id: str, order_line_id: str, lot_code: str, quantity: float, actor: str, package_id: str = "", location_code: str = "RECEIVING", reference: str = "") -> InventoryTransaction:
        ...
        with self._session_factory.begin() as session:
            ...
            duplicate = session.scalar(select(InventoryLot.id).where(
                ...
                or_(InventoryLot.lot_code == clean_lot, InventoryLot.compliance_package_id == str(package_id).strip()) if str(package_id).strip() else InventoryLot.lot_code == clean_lot,
            ))
            if duplicate:
    ...
    def list_commercial_transactions(self, organization_id: str, facility_id: str) -> list[InventoryTransaction]:
        with self._session_factory() as session:
            return list(session.scalars(select(InventoryTransaction).where(
    ...
    def _update_order_status(self, order_id: str, *, organization_id: str, facility_id: str, status: str, actor: str) -> CommercialOrder:
        with self._session_factory.begin() as session:
            order = self._require_order(session, order_id, organization_id, facility_id)
            ...
            order.status = status
            order.updated_by = actor
            self._audit(session, organization_id, facility_id, "commercial_order", order.id, "confirmed", actor, {})
            return order

    @staticmethod
    def _lot_balance(session: Session, organization_id: str, lot_id: str) -> float:
        return float(session.scalar(select(func.coalesce(func.sum(InventoryTransaction.quantity_delta), 0.0)).where(
            InventoryTransaction.organization_id == organization_id,
            InventoryTransaction.lot_id == lot_id,
        )) or 0.0)

    @staticmethod
    def _require_order(session: Session, order_id: str, organization_id: str, facility_id: str) -> CommercialOrder:
    ...
    @staticmethod
    def _audit(session: Session, organization_id: str, facility_id: str | None, entity_type: str, entity_id: str, action: str, actor: str, changes: dict[str, Any]) -> None:
