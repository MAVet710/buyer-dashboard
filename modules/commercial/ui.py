    ...
    st.markdown(
        ...
        <div class="commercial-order-card">
            ...
            <div class="commercial-order-card__meta">
                {_money(value)} 路 Due {html.escape(due)} 路 {shipped:,.0f} / {requested:,.0f} fulfilled
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def _dashboard(
    ...
    with exceptions:
        ...
        for lot, balance, product_name in inventory_exceptions[:5]:
            st.warning(f"{product_name} 路 lot {lot.lot_code} 路 {balance:,.2f} on hand")
...
def _new_order(
    ...
    with st.form("commercial_new_order", clear_on_submit=True):
        ...
        default_lines = pd.DataFrame(
            [
                {
                    "Product": f"{seed_product.sku} 路 {seed_product.name}",
                    "Quantity": 1.0,
        ...
        product_labels = [f"{product.sku} 路 {product.name}" for product in products]
    ...
    if submitted:
        if partner is None:
            ...
            return
        by_label = {f"{product.sku} 路 {product.name}": product for product in products}
        payload = []
                ...
                payload.append(
                    {
                     ...
                     阄mР陬澠瓂襙lines = [line for line in lines if line.commercial_order_id == order.id]
    products_by_id = {product.id: product for product in products}
    st.dataframe(
        pd.DataFrame(
            [
    ...
    line = st.selectbox(
        "Line to process",
        remaining_lines,
        format_func=lambda row: f"{row.position}. {getattr(products_by_id.get(row.product_id), 'name', row.description)} 路 {row.quantity - row.fulfilled_quantity:,.2f} remaining",
    )
    ...
    lot = st.selectbox(
        "Inventory lot",
        matching_lots,
        format_func=lambda row: f"{row.lot_code} 路 {coman.inventory_balance(organization_id, row.id):,.2f} {line.unit} on hand",
    )
