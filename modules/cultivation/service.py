...
from .models import CultivationCostEntry, CultivationHarvestPlant, CultivationPlant, CultivationPlantEvent, CultivationRoom
...
class CultivationService:
    ...
    def list_plants(self, organization_id: str, facility_id: str, phase: str = "", room: str = "", search: str = ""):
        with self.sessions() as session:
            ...
            if phase:
                statement = statement.where(CultivationPlant.phase == phase)
            if room:
                statement = statement.where(CultivationPlant.room_code == room)
    ...
    def create_plant(self, organization_id: str, facility_id: str, *, plant_tag: str, strain_name: str, phase: str, room_code: str, actor: str, source_lot_id: str | None = None, mother_plant_tag: str = "", planted_at: date | None = None, estimated_harvest_date: date | None = None, notes: str = ""):
        ...
        if not tag or not strain_name.strip():
            raise ValueError("Plant tag and strain are required.")
        ...
        with self.sessions.begin() as session:
            ...
            if source_lot_id:
                ...
                if not lot or lot.organization_id != organization_id or lot.facility_id != facility_id:
                    raise ValueError("Source package was not found in the active facility.")
            plant = CultivationPlant(
                ...
                phase=phase,
                room_code=room_code.strip() or "UNASSIGNED",
                source_lot_id=source_lot_id,
                mother_plant_tag=mother_plant_tag.strip(),
    ...
    def transition(self, organization_id: str, facility_id: str, plant_id: str, *, phase: str, room_code: str | None, actor: str, reason: str = "", notes: str = ""):
        ...
        with self.sessions.begin() as session:
            ...
            if target != plant.phase:
                if target not in TRANSITIONS.get(plant.phase, set()):
                    raise ValueError(f"Plant cannot move from {plant.phase} to {target}.")
    ...
    def upsert_room(self, organization_id: str, facility_id: str, *, room_code: str, display_name: str = "", phase: str = "", plant_capacity: int = 0, square_feet: float = 0, target_cycle_days: int = 0, active: bool = True, notes: str = "") -> dict:
        ...
        if normalized_phase and normalized_phase not in TRANSITIONS:
            raise ValueError("Room phase must be a supported cultivation phase.")
        if plant_capacity < 0 or square_feet < 0 or target_cycle_days < 0:
            raise ValueError("Room capacity, square feet and cycle days cannot be negative.")
        with self.sessions.begin() as session:
    ...
    def list_harvests(self, organization_id: str, facility_id: str) -> list[dict]:
        with self.sessions() as session:
            harvests = list(session.scalars(select(CultivationHarvest).where(CultivationHarvest.organization_id == organization_id, CultivationHarvest.facility_id == facility_id).order_by(CultivationHarvest.created_at.desc())))
        ...
        for cost in costs:
            costs_by_harvest.setdefault(cost.entity_id, []).append(cost)
        return [self._harvest_payload(row, plant_counts.get(row.id, int(row.plant_count or 0)), costs_by_harvest.get(row.id, [])) for row in harvests]
    ...
    def harvest_detail(self, organization_id: str, facility_id: str, harvest_id: str) -> dict:
        ...
        return self._harvest_payload(harvest, len(links) or int(harvest.plant_count or 0), costs) | {
            "plants": [self._plant_payload(row) for row in plants],
            "cost_entries": [self._cost_payload(row) for row in costs],
        }

    def create_harvest(self, organization_id: str, facility_id: str, *, harvest_code: str, plant_ids: list[str], actor: str, notes: str = "") -> dict:
        code = harvest_code.strip()
        unique_ids = list(dict.fromkeys(str(value).strip() for value in plant_ids if str(value).strip()))
        if not code:
            raise ValueError("Harvest code is required.")
        if not unique_ids:
            raise ValueError("Select at least one flowering plant for the harvest.")
        with self.sessions.begin() as session:
            ...
            if len(plants) != len(unique_ids):
                raise ValueError("One or more selected plants were not found in the active facility.")
            invalid = [plant.plant_tag for plant in plants if plant.phase != "flowering"]
            if invalid:
                raise ValueError(f"Only flowering plants can be assigned to a harvest: {', '.join(invalid[:5])}.")
            ...
            if existing:
                raise ValueError("One or more selected plants are already assigned to an open harvest.")
            ...
            harvest = CultivationHarvest(
                ...
                facility_id=facility_id,
                harvest_code=code,
                strain=next(iter(strains)) if len(strains) == 1 else "Mixed",
                room=next(iter(rooms)) if len(rooms) == 1 else "Mixed",
    ...
    def transition_harvest(self, organization_id: str, facility_id: str, harvest_id: str, *, status: str, actor: str, wet_weight: float | None = None, dry_weight: float | None = None, waste_weight: float | None = None, unit: str = "", notes: str = "") -> dict:
        ...
        if unit.strip() and unit.strip().casefold() not in {"g", "gram", "grams"}:
            raise ValueError("Harvest 360 stores canonical harvest weights in grams.")
        with self.sessions.begin() as session:
            harvest = session.get(CultivationHarvest, harvest_id)
            if not harvest or harvest.organization_id != organization_id or harvest.facility_id != facility_id:
                raise ValueError("Harvest was not found in the active facility.")
            if target != harvest.status and target not in HARVEST_TRANSITIONS.get(harvest.status, set()):
                raise ValueError(f"Harvest cannot move from {harvest.status} to {target}.")
            for value, label in ((wet_weight, "wet weight"), (dry_weight, "dry weight"), (waste_weight, "waste weight")):
                if value is not None and float(value) < 0:
                    raise ValueError(f"Harvest {label} cannot be negative.")
            if wet_weight is not None:
                harvest.wet_weight_g = float(wet_weight)
            if dry_weight is not None:
                harvest.dry_weight_g = float(dry_weight)
            if waste_weight is not None:
                harvest.waste_weight_g = float(waste_weight)
            if notes.strip():
                harvest.notes = notes.strip()
    ...
    def add_cost(self, organization_id: str, facility_id: str, *, entity_type: str, entity_id: str, cost_type: str, actor: str, description: str = "", quantity: float = 0, unit: str = "", unit_cost: float = 0, amount: float | None = None, occurred_on: date | None = None, notes: str = "") -> dict:
        entity = entity_type.strip().casefold()
        kind = cost_type.strip().casefold()
        if entity not in COST_ENTITY_TYPES:
            raise ValueError("Unsupported cultivation cost entity.")
        if kind not in COST_TYPES:
            raise ValueError("Cultivation cost type must be labor, material or overhead.")
        quantity = float(quantity or 0)
        unit_cost = float(unit_cost or 0)
        calculated = quantity * unit_cost if amount is None else float(amount)
        if quantity < 0 or unit_cost < 0 or calculated < 0:
            raise ValueError("Cultivation cost values cannot be negative.")
        with self.sessions.begin() as session:
            ...
            session.add(row)
            session.flush()
            if entity == "harvest" and kind == "labor" and unit.strip().casefold() in {"hr", "hrs", "hour", "hours"}:
                harvest = session.get(CultivationHarvest, entity_id)
                if harvest is not None:
                    harvest.labor_hours = float(harvest.labor_hours or 0) + quantity
            payload = self._cost_payload(row)
        return payload

    @staticmethod
    def _require_cost_entity(session, organization_id: str, facility_id: str, entity_type: str, entity_id: str) -> None:
        ...
        if not row or row.organization_id != organization_id or row.facility_id != facility_id:
            raise ValueError(f"Cultivation {entity_type} was not found in the active facility.")
    ...
    @staticmethod
    def _harvest_payload(harvest: CultivationHarvest, plant_count: int, costs: list[CultivationCostEntry]) -> dict:
        by_type = Counter()
        for cost in costs:
            by_type[cost.cost_type] += float(cost.amount or 0)
        total = sum(by_type.values())
        wet = float(harvest.wet_weight_g or 0)
        dry = float(harvest.dry_weight_g or 0)
        waste = float(harvest.waste_weight_g or 0)
        return {
            "id": harvest.id,
            "harvest_code": harvest.harvest_code,
            "strain_name": harvest.strain,
            "room_code": harvest.room,
            "status": harvest.status,
            "planned_date": None,
            "started_at": harvest.harvested_at.isoformat() if harvest.harvested_at else None,
            "finished_at": harvest.completed_at.isoformat() if harvest.completed_at else None,
            "plant_count": plant_count,
            "wet_weight": wet,
            "dry_weight": dry,
            "waste_weight": waste,
            "unit": "g",
            "dry_yield_pct": round((dry / wet * 100), 2) if wet > 0 else None,
            "labor_hours": round(float(harvest.labor_hours or 0), 2),
            "labor_cost_usd": round(by_type.get("labor", 0.0), 2),
            "material_cost_usd": round(by_type.get("material", 0.0), 2),
            "overhead_cost_usd": round(by_type.get("overhead", 0.0), 2),
            "total_cogs_usd": round(total, 2),
            "cost_per_dry_unit": round(total / dry, 4) if dry > 0 else None,
            "notes": harvest.notes,
