...
def setup(tmp_path):
    ...
    names = {"coman_organizations", "coman_facilities", "coman_products", "coman_inventory_lots",
             ...
             "inventory_audits", "commercial_orders"}
...
def test_manual_notes_and_automatic_override_guard(setup):
    engine, context = setup
    annotate("permissions", AnnotationInput(notes="Reviewed assigned roles", owner="Operations", manual_status="complete"), context, engine)
    ...
    with Session(engine) as session:
        assert session.scalar(select(ReadinessAnnotation)).owner == "Operations"
...
def test_migration_matches_models_and_preserves_evidence(tmp_path):
    ...
    migration = importlib.import_module("migrations.versions.0080_onboarding_reporting")
