...
def _guard() -> tuple[CultivationRegulatoryGuard, TraceabilityObjectLinkRepository]:
    ...
    return CultivationRegulatoryGuard(engine), TraceabilityObjectLinkRepository(engine)
...
def test_verified_tracked_plant_blocks_local_only_mutation():
    guard, repo = _guard()
    _link(repo, "plant-1")
    with pytest.raises(ValueError, match="verified Metrc identity"):
        guard.require_local_only_allowed(
            organization_id="org-1",
            facility_id="fac-1",
            entity_type="cultivation_plant",
