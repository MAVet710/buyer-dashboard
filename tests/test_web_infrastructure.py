...
def test_fresh_database_migrates_to_head_and_latest_revision_rolls_back(tmp_path, monkeypatch):
    ...
    with engine.connect() as connection:
        assert MigrationContext.configure(connection).get_current_revision() == "0057_receiving_discrepancies"

    command.downgrade(config, "-1")
    with engine.connect() as connection:
        assert MigrationContext.configure(connection).get_current_revision() == "0056_trace_reconciliation"
