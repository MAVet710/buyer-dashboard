...
ENCRYPTION_KEY = "phase7-test-integration-key"
...
def _seed():
    ...
    vendor = repo.create_trade_partner(
        organization_id,
        name="Supply Vendor",
        partner_type="vendor",
        actor="buyer-1",
    ...
    order = repo.create_order(
        ...
        lines=[{
            ...
            "description": "Packaging Jar",
            "quantity": 100,
            "unit": "unit",
            "unit_price": 1.25,
        }],
        actor="buyer-1",
        external_reference="Vendor quote 88",
        notes="Human-approved purchasing order",
    )
    return engine, repo, organization_id, facility_id, product_id, vendor.id, order.id


def _offline_connection(service):
    service._connection = lambda organization_id, facility_id, actor: ("test-access-token", {  # type: ignore[method-assign]
        "realm_id": "realm-1",
        "environment": "sandbox",
        "api_base_url": "",
    })
...
def test_vendor_sync_is_idempotent_and_uses_existing_accounting_link_ledger(monkeypatch):
    ...
    _offline_connection(service)
    calls = []

    def fake_request(**kwargs):
        calls.append(kwargs)
        assert kwargs["entity"] == "vendor"
        assert kwargs["payload"]["DisplayName"] == "Supply Vendor"
        return {"Vendor": {"Id": "VENDOR-1", "SyncToken": "0"}}

    monkeypatch.setattr(qbo_purchasing, "quickbooks_api_request", fake_request)
    first = service.sync_vendor(
        organization_id=organization_id,
        facility_id=facility_id,
        partner_id=vendor_id,
        actor="admin-1",
    )
    second = service.sync_vendor(
        organization_id=organization_id,
        facility_id=facility_id,
        partner_id=vendor_id,
        actor="admin-1",
    ...
    with Session(engine) as session:
        ...
        assert link.external_id == "VENDOR-1"
...
def test_confirmed_purchase_order_posts_deterministic_item_lines_and_skips_unchanged_replay(monkeypatch):
    ...
    def fake_request(**kwargs):
        calls.append(kwargs)
        assert kwargs["entity"] == "purchaseorder"
        payload = kwargs["payload"]
        assert payload["VendorRef"] == {"value": "VENDOR-1"}
        assert payload["DocNumber"] == "PO-1001"
        assert payload["TxnDate"] == "2026-08-30"
        assert len(payload["Line"]) == 1
        line = payload["Line"][0]
        assert line["Amount"] == 125.0
        assert line["DetailType"] == "ItemBasedExpenseLineDetail"
        assert line["ItemBasedExpenseLineDetail"] == {
    ...
    monkeypatch.setattr(qbo_purchasing, "quickbooks_api_request", fake_request)
    first = service.sync_purchase_order(
        organization_id=organization_id,
        facility_id=facility_id,
        order_id=order_id,
        actor="admin-1",
    )
    second = service.sync_purchase_order(
        organization_id=organization_id,
        facility_id=facility_id,
        order_id=order_id,
        actor="admin-1",
...
def test_reconciliation_is_read_only_and_reports_local_mapping_or_change_state(monkeypatch):
    ...
    assert initial["vendors"][0]["status"] == "never_synced"
    assert initial["purchase_orders"][0]["sync_status"] == "blocked_vendor_mapping"
    ...
    with Session(engine) as session, session.begin():
        service._upsert_link(
            ...
            internal_id=vendor_id,
            external_id="VENDOR-1",
            payload_hash="stale-vendor-hash",
            actor="admin-1",
        )
    mapped_vendor = service.reconciliation_snapshot(organization_id, facility_id)
    assert mapped_vendor["vendors"][0]["status"] == "local_changes_pending"
    assert mapped_vendor["purchase_orders"][0]["sync_status"] == "blocked_item_mapping"
    ...
    mapped_items = service.reconciliation_snapshot(organization_id, facility_id)
    assert mapped_items["purchase_orders"][0]["sync_status"] == "never_synced"
