...
def test_capabilities_expand_only_for_directly_verified_jurisdictions():
    assert len(DOCUMENTATION_VERIFIED_JURISDICTIONS) == 23
    assert {"MA", "OR", "KY", "LA", "RI"}.issubset(DOCUMENTATION_VERIFIED_JURISDICTIONS)
    assert "AL" not in DOCUMENTATION_VERIFIED_JURISDICTIONS

    assert capability_status("OR", "packages") == CapabilityStatus.JURISDICTION_SPECIFIC
    assert capability_status("RI", "packages") == CapabilityStatus.JURISDICTION_SPECIFIC
    assert capability_status("AL", "packages") == CapabilityStatus.UNKNOWN
    assert require_capability("RI", "packages", environment="production") == CapabilityStatus.JURISDICTION_SPECIFIC
    with pytest.raises(ValueError, match="unknown/unverified"):
        require_capability("AL", "packages", environment="production")
...
def test_capability_evidence_is_sourceable_and_sandbox_stays_environment_gated():
    evidence = capability_evidence("RI", "transfers")
    assert evidence is not None
    assert evidence.endpoint == "GET /transfers/v2/incoming"
    assert evidence.source_url == "https://api-ri.metrc.com/Documentation/"
    assert evidence.verified_on == "2026-08-28"

    ri = get_jurisdiction("RI")
    al = get_jurisdiction("AL")
    assert ri is not None and ri.documentation_verified is True
    assert al is not None and al.documentation_verified is False

    assert capability_status("RI", "sandbox_setup") == CapabilityStatus.SANDBOX_ONLY
    assert require_capability("RI", "sandbox_setup", environment="sandbox") == CapabilityStatus.SANDBOX_ONLY
    with pytest.raises(ValueError, match="sandbox-only"):
        require_capability("RI", "sandbox_setup", environment="production")
    with pytest.raises(ValueError, match="sandbox is not verified"):
        require_capability("AL", "facilities", environment="sandbox")
