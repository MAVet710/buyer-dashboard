...
async function fixture(page: Page, pipeline = false) {
  ...
  let relationship: Record<string, unknown> = { id: "account", partner_id: "customer", owner_user_id: "rep", owner_name: "Rep", status: "active", next_action: "Call buyer", next_action_date: "2026-10-01" };
  const quotes: Record<string, unknown>[] = [];
  const opportunities: Record<string, unknown>[] = [{ id: "opportunity", partner_id: "customer", customer_name: "Retail Account", title: "Restock", stage: "qualified", estimated_value: 200, expected_close_date: null, owner_user_id: "rep", owner_name: "Rep", source: "Referral", next_action: "Send quote", next_action_date: null }];
  await page.route("**/api/v1/**", async route => {
    ...
    if (request.method() === "POST") {
      ...
      if (path.endsWith("/follow-ups")) body = { id: "work-follow-up" };
    ...
    else if (path.endsWith("/owners")) body = { items: [{ id: "rep", name: "Rep" }, { id: "new-rep", name: "New rep" }], has_more: false };
    else if (path.endsWith("/products")) body = [{ id: "product", name: "Flower", sku: "SKU", base_unit: "unit" }];
...
test("account edits, quotes and activities preserve CRM payloads and canonical order handoff", async ({ page }) => {
  ...
  await page.getByRole("combobox", { name: "Assigned user", exact: true }).first().selectOption("new-rep");
  await page.getByRole("button", { name: "Save account" }).click();
  await expect.poll(() => writes.length).toBe(1);
  expect(writes[0].payload).toEqual({ owner_user_id: "new-rep", status: "active", next_action: "Call buyer", next_action_date: "2026-10-01" });
  ...
  await page.getByLabel("Follow-up title").fill("Call about restock");
  await page.getByLabel("Follow-up opportunity").selectOption("opportunity");
  await page.getByRole("button", { name: "Create Work follow-up" }).click();
  await expect.poll(() => writes.length).toBe(5);
  expect(writes[4].payload).toEqual({ title: "Call about restock", opportunity_id: "opportunity", assignee_id: null, due_at: null });
  await expect(page.getByRole("status")).toContainText("work-follow-up");
});
...
test("Work customer/opportunity return route opens the linked account and edit form", async ({ page }) => {
  await fixture(page);
  await page.goto(origin + "/e2e/fixtures/wholesale-crm.html?tab=customers&customer=customer&opportunity=opportunity");
  await expect(page.getByText("CUSTOMER 360", { exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Edit opportunity" })).toBeVisible();
  await expect(page.getByLabel("Title", { exact: true })).toHaveValue("Restock");
});
