...
const STAGED_TRANSFER_KEY="buyer-dash-transfer-package-selection";
...
export function OrdersPage(){
  ...
  return <div className="page commercial-workspace">
    ...
    <div className="commercial-header"><div><div className="commercial-eyebrow">Wholesale operations</div><h1 className="commercial-title">Orders & Fulfillment</h1><p className="commercial-subtitle">Reserve the exact package, then carry it into the licensed transfer without re-entering the order handoff.</p></div><span className="commercial-live">{data?.facility_name??"Active facility"}</span></div>
...
function Fulfillment({data,detail,onSaved}:{data:Workspace;detail:Detail;onSaved:()=>void}){
  ...
  const postDirect=useMutation({mutationFn:()=>apiPost(`/api/v1/commercial/order-lines/${line?.id}/fulfill`,{lot_id:lotId||lots[0]?.id,quantity,reference}),onSuccess:onSaved});
  ...
  const selectedLotRow=lots.find(row=>row.id===selectedLot)??lots[0];
  ...
  function prepareLicensedTransfer(){
    sessionStorage.setItem(STAGED_TRANSFER_KEY,JSON.stringify({
      operation:"production",
      lot_ids:[selectedLot],
      quantities:{[selectedLot]:quantity},
      commercial_order_lines:{[selectedLot]:line.id},
      fulfillment_reference:reference,
      order_number:detail.order.order_number,
      package_label:selectedLotRow.compliance_package_id||selectedLotRow.lot_code,
      source:"wholesale",
    }));
    window.location.assign("/production/inventory/transfers");
  }
  return <div className="production-controls"><label>Line to process<select value={line.id} onChange={event=>{const next=remaining.find(row=>row.id===event.target.value);setLineId(event.target.value);setLotId("");setQuantity(next?Math.min(1,next.quantity-next.fulfilled_quantity):0)}}>{remaining.map(row=><option value={row.id} key={row.id}>{row.position}. {data.products.find(product=>product.id===row.product_id)?.name??row.description} · {number(row.quantity-row.fulfilled_quantity)} remaining</option>)}</select></label><label>Inventory lot<select value={selectedLot} onChange={event=>setLotId(event.target.value)}>{lots.map(row=><option value={row.id} key={row.id}>{row.lot_code} · {number(row.on_hand)} {line.unit} on hand</option>)}</select></label><label>Quantity<input type="number" min="0.0001" max={Math.max(.0001,line.quantity-line.fulfilled_quantity)} value={quantity} onChange={event=>setQuantity(Number(event.target.value))}/></label><label>Fulfillment reference<input value={reference} onChange={event=>setReference(event.target.value)}/></label>{detail.order.order_type==="sales"?<><div className="audit-actions"><button className="primary" onClick={()=>reserve.mutate()}>Reserve lot</button><button className="primary" disabled={reserved+1e-9<quantity} onClick={prepareLicensedTransfer}>Prepare licensed transfer</button></div><p className="section-note">{number(reserved)} {line.unit} reserved from this package.</p><div className="info-banner"><strong>Next action:</strong> Prepare the licensed transfer. DoobieLogic will carry the package, quantity, sales-order line, and fulfillment reference forward. You only choose the destination, enter the state manifest, confirm Metrc, and post the transfer once.</div><details className="streamlit-expander"><summary>Non-license shipment exception</summary><div className="streamlit-expander-body"><div className="warning-banner">Use this only when the physical inventory is leaving without a DoobieLogic cross-license transfer. Do not use both actions for the same cannabis.</div><button className="secondary" disabled={reserved+1e-9<quantity} onClick={()=>postDirect.mutate()}>Post non-transfer shipment</button></div></details></>:<button className="primary submit" onClick={()=>postDirect.mutate()}>Post receipt</button>}{reserve.isSuccess?<div className="success-banner">Inventory reserved. Next: prepare the licensed transfer.</div>:null}{postDirect.isSuccess?<div className="success-banner">{detail.order.order_type==="sales"?"Non-transfer shipment posted to the immutable inventory ledger.":"Receipt posted to inventory."}</div>:null}{reserve.isError||postDirect.isError?<div className="form-error">{reserve.error?.message??postDirect.error?.message}</div>:null}</div>;
}
...
function Ledger({rows}:{rows:Transaction[]}){return <section className="inventory-panel"><h2>Commercial inventory ledger</h2><p>Receipts and physical movements are append-only. Licensed sales transfers are represented by the transfer-out ledger movement rather than a duplicate wholesale shipment decrement.</p>{rows.length?<><DataTable rows={rows.map(row=>({Occurred:row.occurred_at,Order:row.order,Type:row.type,"Product Name":row.product_name,Lot:row.lot,Quantity:row.quantity,Unit:row.unit,Reference:row.reference,Actor:row.actor}))}/><button className="secondary" onClick={()=>downloadCsv(rows)}>Export ledger CSV</button></>:<div className="info-banner">No commercial receipts or shipments have been posted yet.</div>}</section>}
