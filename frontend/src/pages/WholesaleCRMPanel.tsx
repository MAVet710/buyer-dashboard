...
type Account = { owner: string; status: string; next_action: string; next_action_date: string | null };
...
type Opportunity = { id: string; partner_id: string; customer_name?: string; title: string; stage: string; estimated_value: number; expected_close_date: string | null; owner: string; source: string; next_action: string; next_action_date: string | null };
...
type Product = { id: string; name: string; sku: string; base_unit: string; retail_price: number };
type Detail = { customer: Customer; relationship: Account | null; sales_value: number; open_balance: number; opportunities: Opportunity[]; quotes: Quote[]; pricing: { id: string; product_id: string; price_usd: number; discount_pct: number }[]; timeline: { id: string; kind: string; at: string; text: string; status: string; actor: string }[] };
type Page<T> = { items: T[]; has_more: boolean };
const money = (n: number) => n.toLocaleString(undefined, { style: "currency", currency: "USD" });
const emptyAccount: Account = { owner: "", status: "prospect", next_action: "", next_action_date: null };
...
export function WholesaleCRMPanel({ pipeline = false }: { pipeline?: boolean }) {
  ...
  return <section className="inventory-panel">
    ...
    <div className="table-wrap"><table><thead><tr><th>Customer</th>{pipeline && <th>Opportunity</th>}<th>{pipeline ? "Stage / Value" : "Status / Terms"}</th><th>Assigned rep</th><th>Next action</th><th>Due</th></tr></thead><tbody>
      {pipeline ? opportunities.data?.items.map(o => <tr key={o.id}><td><button className="secondary" onClick={() => setSelected(o.partner_id)}>{o.customer_name}</button></td><td>{o.title}</td><td>{o.stage} / {money(o.estimated_value)}</td><td>{o.owner || "Unassigned"}</td><td>{o.next_action || "Set next action"}</td><td>{o.next_action_date || "Not scheduled"}</td></tr>) : customers.data?.items.map(c => <tr key={c.id}><td><button className="secondary" onClick={() => setSelected(c.id)}>{c.name}</button></td><td>{c.relationship?.status || "prospect"} / {c.payment_terms}</td><td>{c.relationship?.owner || "Unassigned"}</td><td>{c.relationship?.next_action || "Set next action"}</td><td>{c.relationship?.next_action_date || "Not scheduled"}</td></tr>)}
    </tbody></table></div>
...
function Customer360({ partnerId, onBack }: { partnerId: string; onBack: () => void }) {
  ...
  const mutate = useMutation({ mutationFn: ({ path, payload }: { path: string; payload: unknown }) => apiPost<{ id?: string }>(root + path, payload), onSuccess: async (result, variables) => {
    setMessage(variables.path.endsWith("/convert") ? `Draft sales order created or recovered: ${result.id}. Continue in Orders.` : "Saved.");
  ...
  return <section className="inventory-panel crm-workspace">
    ...
    {!d.timeline.length && <p>No activity yet.</p>}<ol>{d.timeline.map(e => <li key={`${e.kind}-${e.id}`}><strong>{e.kind.replaceAll("_", " ")}: {e.text}</strong><p>{new Date(e.at).toLocaleString()} | {e.status} {e.actor && `| ${e.actor}`}</p></li>)}</ol>
  </section>;
}

function AccountForm({ account, busy, onSave }: { account: Account; busy: boolean; onSave: (v: Account) => void }) {
  const [v, set] = useState(account);
  return <form className="crm-form" onSubmit={e => { e.preventDefault(); onSave({ owner: v.owner, status: v.status, next_action: v.next_action, next_action_date: v.next_action_date }); }}><h3>Account next action</h3><label>Assigned rep <input maxLength={255} value={v.owner} onChange={e => set({ ...v, owner: e.target.value })} /></label><label>Status <select value={v.status} onChange={e => set({ ...v, status: e.target.value })}>{["prospect", "active", "on_hold", "inactive"].map(s => <option key={s}>{s}</option>)}</select></label><label>Next action <input maxLength={1000} value={v.next_action} onChange={e => set({ ...v, next_action: e.target.value })} /></label><label>Next action date <input type="date" value={v.next_action_date || ""} onChange={e => set({ ...v, next_action_date: e.target.value || null })} /></label><button disabled={busy}>Save account</button></form>;
}
...
function OpportunityForm({ opportunity, busy, onSave, onCancel }: { opportunity: Opportunity | null; busy: boolean; onSave: (v: unknown) => void; onCancel: () => void }) {
  const [v, set] = useState<Omit<Opportunity, "id" | "partner_id">>(opportunity || { title: "", stage: "lead", estimated_value: 0, expected_close_date: null, owner: "", source: "", next_action: "", next_action_date: null });
  function submit(e: FormEvent) { e.preventDefault(); const { title, stage, estimated_value, expected_close_date, owner, source, next_action, next_action_date } = v; onSave({ title, stage, estimated_value, expected_close_date, owner, source, next_action, next_action_date }); }
...
function QuoteForm({ partnerId, opportunities, busy, onSave }: { partnerId: string; opportunities: Opportunity[]; busy: boolean; onSave: (v: unknown) => void }) {
  ...
  return <form className="crm-form" onSubmit={e => { e.preventDefault(); onSave({ title, opportunity_id: opportunity || null, lines: lines.map(l => ({ product_id: l.product_id, quantity: l.quantity, unit_price: l.price === "" ? null : Number(l.price) })) }); }}>
    ...
    <p>Leave price blank to use customer pricing, then the product retail price. Review saved quote prices before conversion.</p>
