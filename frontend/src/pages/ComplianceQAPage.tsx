...
export function ComplianceQAPage() {
  ...
  const canPublish = ["dev", "admin", "qa", "supervisor"].includes(account.data?.user.role ?? "");
  ...
  return <div className="page">
    ...
    <div className="two-column-grid">
      ...
      <section className="inventory-panel"><div className="section-heading"><div><div className="eyebrow">{canPublish ? "Compliance QA administration" : "Compliance source"}</div><h3>Reviewed source file</h3></div></div><p className="muted">{status.data?.configured ? `${status.data.filename} · ${status.data.rows.toLocaleString()} rows · ${status.data.quality ?? "Active"}` : "No active compliance source is available in this facility."}</p><div className="form-actions"><button className="secondary" onClick={template}>Download template</button>{canPublish ? <label className="file-drop inline-file">Publish reviewed source<input type="file" accept=".csv,.xlsx,.xls" onChange={event => upload(event.target.files?.[0])} /></label> : null}</div>{canPublish ? <p className="muted">Publishing is restricted to compliance QA, supervisors, company admins and Level DEV.</p> : <p className="muted">Ask a compliance QA user, supervisor or administrator to publish a reviewed source revision.</p>}{status.data?.error ? <div className="form-error">{status.data.error}</div> : null}</section>
    </div>
