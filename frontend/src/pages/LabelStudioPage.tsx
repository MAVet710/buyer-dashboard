...
type InventoryLabelSource = {
  ...
  on_hand:number;inventory_unit:string;label:Record<string,string>;coa:CoaSource;qr:{value:string;svg:string};raw_text:string;
...
export function LabelStudioPage(){
  ...
  const canPrint=Boolean(Object.keys(fields).length&&!missingRequired.length&&review.data?.status==="pass"&&!activeSource?.coa.needs_confirmation&&activeSource?.package_id);
  return <div className="page"><style>{`@media print{body *{visibility:hidden!important}.label-print-preview,.label-print-preview *{visibility:visible!important}.label-print-preview{position:absolute!important;left:0!important;top:0!important;width:100%!important;max-width:none!important;margin:0!important;border:0!important;box-shadow:none!important;background:#fff!important;color:#000!important}.label-print-preview .label-qr-image{width:1in!important;height:1in!important}}.label-qr-image{width:120px;height:120px;display:block}.label-traceability{font-family:monospace;word-break:break-all}.locked-field{background:rgba(127,127,127,.08)}`}</style><div className="eyebrow">COMPLIANCE · LABELS</div><h1>Label Studio + LabelGuard</h1><p className="section-note">Select an inventory batch to build the label from Product Master, the active facility, its current METRC package tag, and the COA attached to that tested material. A split or repack may have a new current tag while retaining the verified ancestor COA through package lineage. The QR and printed package identity always use the current tag.</p>
    <div className="view-tabs parity-tabs"><button className="active" type="button">Build & review</button><button type="button" disabled={!canPrint} title={canPrint?"Print the reviewed label with the current METRC package QR":activeSource?.coa.needs_confirmation?"Confirm the pending COA association before printing":missingRequired.length?`Complete required fields: ${missingRequired.map(fieldLabel).join(", ")}`:review.data?.status==="warning"?"Resolve the warning or obtain an approved print override workflow":"Run LabelGuard and receive a PASS before printing"} onClick={()=>window.print()}>Print reviewed label</button></div>

    <section className="inventory-panel"><div className="eyebrow">BUILD FROM INVENTORY</div><h2>Select a batch or package</h2><p className="section-note">Only on-hand inventory in the active facility is available here. The selected lot's current METRC tag is immutable inside Label Studio and becomes both the printed traceability ID and QR payload.</p>
    ...
    {activeSource?<section className="inventory-panel"><div className="eyebrow">COA SOURCE</div><h2>Test results for this material</h2>
      ...
      {activeSource.coa.available&&inheritedCoa?<><div className="success-banner"><strong>COA inherited through package lineage.</strong> Current package <span className="label-traceability">{activeSource.package_id}</span> retains the verified COA from tested package <span className="label-traceability">{activeSource.coa.metrc_source_id}</span>. The printed label and QR stay tied to the current package tag.</div><div className="audit-actions"><button className="secondary" type="button" onClick={()=>void openCoa()}>Open source COA</button></div></>:null}
      ...
      {!activeSource.package_id?<div className="warning-banner"><strong>No METRC package/tag is stored on this inventory lot.</strong> Automatic COA resolution, QR generation, and printing stay disabled until traceability identity is present.</div>:null}
