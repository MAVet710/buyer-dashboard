...
type SandboxContext = {
  canonical_dev_sandbox:boolean;developer_authorized:boolean;operator_rehearsal_enabled:boolean;label_layout_testing_enabled:boolean;
  local_mutations_enabled:boolean;external_provider_writes_enabled:boolean;provider_transport:string;test_artifact_mark:string;production_guardrails_intact:boolean;
};

const PACKAGING_ONLY_FIELDS = new Set(["warning_text", "universal_symbol", "warning_symbol", "statutory_warning"]);
const COA_OWNED_FIELDS = new Set(["potency","total_thc","total_cbd","total_cannabinoids","total_terpenes","lab_testing_state","laboratory","lab_license_number","test_date","coa_reference"]);
...
export function LabelStudioPage(){
  ...
  const sandbox=useQuery({queryKey:["label-studio-sandbox-context"],queryFn:({signal})=>apiGet<SandboxContext>("/api/v1/label-printing/sandbox-context",signal),retry:false});
  ...
  const sandboxLayoutMode=Boolean(sandbox.data?.label_layout_testing_enabled&&!coaReady);
  const sandboxRequired=useMemo(()=>requiredForReview.filter(field=>!COA_OWNED_FIELDS.has(field)),[requiredForReview]);
  const sandboxMissingRequired=useMemo(()=>sandboxRequired.filter(field=>!String(fields[field]??"").trim()),[fields,sandboxRequired]);
  const compliantCanPrint=Boolean(Object.keys(fields).length&&!missingRequired.length&&review.data?.status==="pass"&&!activeSource?.coa.needs_confirmation&&activeSource?.package_id&&coaReady);
  const sandboxCanPrint=Boolean(sandboxLayoutMode&&Object.keys(fields).length&&!sandboxMissingRequired.length&&activeSource?.package_id&&!activeSource?.coa.needs_confirmation);
  const canPrint=compliantCanPrint||sandboxCanPrint;
  const printTitle=compliantCanPrint?"Print the reviewed testing label with current METRC package QR/barcode":sandboxCanPrint?"Print a clearly marked sandbox layout-test label; this is not a compliance release":activeSource?.coa.needs_confirmation?"Confirm the pending COA association before printing":sandboxLayoutMode&&sandboxMissingRequired.length?`Complete layout-test identity fields: ${sandboxMissingRequired.map(fieldLabel).join(", ")}`:!coaReady?"A verified passing COA with a test date is required before production printing":missingRequired.length?`Complete required fields: ${missingRequired.map(fieldLabel).join(", ")}`:review.data?.status==="warning"?"Resolve the warning or obtain an approved print override workflow":"Run LabelGuard and receive a PASS before printing";
  return <div className="page"><style>{`@media print{body *{visibility:hidden!important}.label-print-preview,.label-print-preview *{visibility:visible!important}.label-print-preview{position:absolute!important;left:0!important;top:0!important;width:100%!important;max-width:none!important;margin:0!important;border:0!important;box-shadow:none!important;background:#fff!important;color:#000!important}.label-print-preview .label-qr-image{width:1in!important;height:1in!important}.label-print-preview .label-barcode-image{width:100%!important;max-width:3in!important;height:auto!important}.sandbox-test-mark{display:block!important;font-size:20pt!important;font-weight:900!important;letter-spacing:.08em!important;border:3px solid #000!important;padding:8px!important;margin:0 0 10px!important;text-align:center!important}}.label-qr-image{width:120px;height:120px;display:block}.label-barcode-image{width:100%;max-width:360px;height:auto;display:block}.label-traceability{font-family:monospace;word-break:break-all}.locked-field{background:rgba(127,127,127,.08)}.sandbox-test-mark{font-weight:900;letter-spacing:.08em;border:2px solid currentColor;padding:8px;margin-bottom:10px;text-align:center}`}</style><div className="eyebrow">COMPLIANCE · TESTING LABELS</div><h1>Label Studio + LabelGuard</h1><p className="section-note">Select an inventory batch to build its testing label from Product Master, the active facility, its current METRC package tag, and the COA attached to that tested material. Packaging-only statutory warning text and warning symbols are intentionally outside this testing-label scope. A split or repack may have a new current tag while retaining the verified ancestor COA through package lineage. The QR and printed package identity always use the current tag.</p>
    {sandbox.data?.label_layout_testing_enabled?<div className="info-banner"><strong>DEV Sandbox operator rehearsal is active.</strong> You can print a visibly marked layout test before a verified COA exists. This does not create a LabelGuard PASS, does not populate missing laboratory facts, and does not enable external Metrc writes.</div>:null}
    <div className="view-tabs parity-tabs"><button className="active" type="button">Build & review</button><button type="button" disabled={!canPrint} title={printTitle} onClick={()=>window.print()}>{sandboxCanPrint&&!compliantCanPrint?"Print sandbox test label":"Print reviewed label"}</button></div>
    ...
    {activeSource?<section className="inventory-panel"><div className="eyebrow">COA SOURCE</div><h2>Test results for this material</h2>
      ...
      {activeSource.coa.available&&!coaReady?<div className="warning-banner"><strong>Testing label cannot be released yet.</strong> The selected material needs a verified passing COA with a test date.</div>:null}
