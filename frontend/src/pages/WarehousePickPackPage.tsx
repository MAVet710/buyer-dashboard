...
import { IdentifierCaptureInput } from "../components/IdentifierCaptureInput";
...
export function WarehousePickPackPage({onNavigate}:{onNavigate:(page:string)=>void}){
  ...
  return <div className="page warehouse-pick-pack"><div className="page-heading"><div><div className="eyebrow">WAREHOUSE · MOBILE PICK / PACK</div><h1>Scan it. Verify it. Move it.</h1><p>DoobieLogic recommends the earliest-expiring available lot first, but the scan is the final identity check before anything is reserved or shipped.</p></div><div className="heading-actions"><button className="secondary" onClick={()=>onNavigate("Orders")}>Orders</button><button className="secondary" onClick={()=>onNavigate("Package 360")}>Package 360</button></div></div>
    ...
    {queue.data?<section className="metrics four"><Metric label="Open sales orders" value={queue.data.open_sales_orders}/><Metric label="Lines to pick" value={queue.data.lines_to_pick}/><Metric label="Current line" value={queue.data.lines_to_pick?`${Math.min(index+1,queue.data.lines_to_pick)} / ${queue.data.lines_to_pick}`:"—"}/><Metric label="Mode" value="FEFO + normalized capture"/></section>:null}
...
function PickCard({line,onDone,onPrevious,onNext}:{line:PickLine;onDone:()=>void;onPrevious:()=>void;onNext:()=>void}){
  ...
  const [scan,setScan]=useState(""); const [scanSource,setScanSource]=useState(""); const [reference,setReference]=useState(line.order_number); const [message,setMessage]=useState("");
  useEffect(()=>{const next=line.recommended_lot??line.available_lots[0];setLotId(next?.id??"");setQuantity(Math.min(line.remaining,next?.on_hand??line.remaining));setScan("");setScanSource("");setMessage("");setReference(line.order_number)},[line.line_id,line.order_number,line.remaining,line.recommended_lot,line.available_lots]);
  const reserve=useMutation({mutationFn:()=>apiPost<PickResult>("/api/v1/warehouse/pick",{order_line_id:line.line_id,lot_id:selected?.id,quantity,scan_code:scan,action:"reserve",reference}),onSuccess:()=>setMessage("Lot reserved. Captured identity remains verified for this selected package.")});
