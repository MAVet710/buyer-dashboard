...
export function TraceabilityActionsPage({onNavigate}:{onNavigate:(page:string)=>void}){
  ...
  const queue=useMutation({mutationFn:()=>apiPost<QueueResult>("/api/v1/traceability-actions/queue",{provider,jurisdiction,environment,operation_type:selected?.operation_type,entity_id:entityId,license_number:license,payload:payload(),reason,idempotency_key:`ui:${selected?.operation_type}:${entityId}:${Date.now()}`})});
