...
import { MetrcPackageControls } from "../components/MetrcPackageControls";
...
type Snapshot = {
  ...
  traceability:{operator_status:string;latest_transaction_id:string;latest_operation:string;provider:string;jurisdiction:string;environment:string;license_number:string;provider_reference:string;attempt_count:number;last_attempt_at:string|null;retry_eligible:boolean;mismatch_reason:string;latest_error:string;actions:{id:string;operation:string;status:string;operator_status:string;provider:string;requested_at:string|null}[]};
...
export function Package360Page({onNavigate,initialCode=""}:{onNavigate:(page:string)=>void;initialCode?:string}){
  ...
  return <div className="page package-360-page">
    ...
    {data?<>
      ...
      <section className="inventory-panel"><div className="eyebrow">COMPLIANCE SYNC</div><h2>{data.traceability.operator_status}</h2><div className="two-column-grid"><div><Row label="Provider" value={data.traceability.provider?data.traceability.provider.toUpperCase():"Not requested"}/><Row label="Latest operation" value={title(data.traceability.latest_operation)}/><Row label="Jurisdiction / environment" value={[data.traceability.jurisdiction,data.traceability.environment].filter(Boolean).join(" · ")||"—"}/><Row label="License" value={data.traceability.license_number||"—"}/></div><div><Row label="Attempts" value={String(data.traceability.attempt_count)}/><Row label="Last attempt" value={date(data.traceability.last_attempt_at)}/><Row label="Provider reference" value={data.traceability.provider_reference||"—"}/><Row label="Retry eligible" value={data.traceability.retry_eligible?"Yes — review required":"No"}/></div></div>{data.traceability.mismatch_reason||data.traceability.latest_error?<div className="warning-banner">{data.traceability.mismatch_reason||data.traceability.latest_error}</div>:null}{data.traceability.latest_transaction_id?<button className="secondary" onClick={()=>onNavigate("Compliance")}>Inspect reconciliation evidence</button>:<div className="info-banner">No state-system action has been requested for this package.</div>}</section>
      <MetrcPackageControls lotId={data.package.id} productId={data.product.id} lotCode={data.package.lot_code} productName={data.product.name} packageLabel={data.package.package_id} localStatus={data.package.status}/>
