...
type StagedTransferDraft = { operation?:string;lot_ids:string[];quantities:Record<string,string>;commercial_order_lines:Record<string,string>;fulfillment_reference:string;order_number:string;package_label:string;source:string };
...
export function InventoryTransferManager({operation,packages,onClose,onSaved,embedded=false}:{operation:Operation;packages:InventoryPackage[];onClose?:()=>void;onSaved?:(message:string)=>void;embedded?:boolean}) {
  ...
  const [staged]=useState<StagedTransferDraft>(()=>readStagedTransfer(operation));
  ...
  const [sourceIds,setSourceIds]=useState<string[]>(()=>packages.length?packages.map(row=>row.id):staged.lot_ids);
  const [destination,setDestination]=useState("");
  const [manifest,setManifest]=useState("");
  const [externalId,setExternalId]=useState(staged.fulfillment_reference);
  const [confirmed,setConfirmed]=useState(false);
  const [quantities,setQuantities]=useState<Record<string,string>>(()=>({
    ...Object.fromEntries(packages.map(row=>[row.id,String(Math.max(0,row.available))])),
    ...staged.quantities,
  }));
  ...
  const allInventory=sourceInventory.data?.items??[];
  const candidates=allInventory.filter(row=>row.available>0&&!["hold","quarantine","failed"].some(token=>row.status.toLowerCase().includes(token)));
  const sourcePackages=packages.length?packages:allInventory.filter(row=>sourceIds.includes(row.id));
  ...
  const dispatchLines=useMemo(()=>sourcePackages.map(row=>({
    source_lot_id:row.id,
    quantity:Number(quantities[row.id]??row.available),
    commercial_order_line_id:staged.commercial_order_lines[row.id]??"",
  })),[quantities,sourcePackages,staged.commercial_order_lines]);
  const dispatchValid=Boolean(canWrite&&destination&&manifest.trim()&&confirmed&&dispatchLines.length&&dispatchLines.every((row,index)=>{
    const packageRow=sourcePackages[index];
    const ownedWholesaleReservation=Boolean(row.commercial_order_line_id);
    const ceiling=ownedWholesaleReservation?packageRow.on_hand:packageRow.available;
    return row.quantity>0&&row.quantity<=ceiling+1e-9;
  }));
  ...
  const dispatch=useMutation({mutationFn:()=>apiPost<Transfer>("/api/v1/inventory/transfers/dispatch",{destination_facility_id:destination,manifest_reference:manifest.trim(),external_transfer_id:externalId.trim(),state_transfer_confirmed:confirmed,lines:dispatchLines,notes:staged.source==="wholesale"?`Licensed fulfillment for wholesale order ${staged.order_number}. Package and quantity were carried from the reservation.`:"Posted from DoobieLogic Inventory transfer workspace."}),onSuccess:row=>{sessionStorage.removeItem(STAGED_SELECTION_KEY);refresh();saved(staged.source==="wholesale"?`Wholesale order ${staged.order_number} fulfilled through transfer ${row.manifest_reference} to ${row.destination_facility_name}.`:`Transfer ${row.manifest_reference} dispatched to ${row.destination_facility_name}.`);setSourceIds([]);setManifest("");setExternalId("");setDestination("");setConfirmed(false);}});
  ...
  const cancel=useMutation({mutationFn:(transferId:string)=>apiPost<Transfer>(`/api/v1/inventory/transfers/${transferId}/cancel`,{reason:cancelReason.trim()||"Cancelled before destination receipt",state_cancel_confirmed:cancelConfirmed}),onSuccess:row=>{setCancelReason("");setCancelConfirmed(false);refresh();saved(`Transfer ${row.manifest_reference} cancelled; source inventory and any wholesale reservation state were restored.`);}});

  const body=<>
    {flash?<div className="success-banner">{flash}</div>:null}
    {staged.source==="wholesale"?<div className="info-banner"><strong>Staged from wholesale order {staged.order_number}.</strong> {staged.package_label||"The reserved package"}, its quantity, sales-order ownership and fulfillment reference were carried forward automatically. Next: choose the destination license, enter the state manifest, confirm the Metrc step, and post the transfer once.</div>:null}
    ...
    <section className="inventory-panel">
      ...
      <button className="primary submit" disabled={!dispatchValid||dispatch.isPending} onClick={()=>dispatch.mutate()}>{staged.source==="wholesale"?"Post licensed transfer & fulfill order":"Post transfer out"}</button>{dispatch.isError?<div className="form-error">{dispatch.error.message}</div>:null}</>:staged.source==="wholesale"&&!sourceInventory.isLoading?<div className="warning-banner">The reserved wholesale package could not be resolved in this facility. Return to the order and review its reservation before shipping.</div>:null}
    </section>
  ...
  if(embedded)return <div className="page"><div className="page-heading"><div><div className="eyebrow">{operation==="production"?"PRODUCTION OPS · INVENTORY":"RETAIL OPS · INVENTORY"}</div><h1>{operation==="production"?"Production Inventory Transfers":"Retail Inventory Transfers"}</h1><p>Move cannabis inventory between licensed facilities without collapsing their physical ledgers. Each transfer preserves manifest, license, package, QA/COA and genealogy context.</p></div><span className="access-badge">{inbound.length} inbound · {outbound.length} outbound</span></div>{body}</div>;
...
function readStagedTransfer(operation:Operation):StagedTransferDraft{try{const raw=sessionStorage.getItem(STAGED_SELECTION_KEY);if(!raw)return emptyStaged();const parsed=JSON.parse(raw) as Record<string,unknown>;if(String(parsed.operation??"")!==operation||!Array.isArray(parsed.lot_ids))return emptyStaged();const quantities=typeof parsed.quantities==="object"&&parsed.quantities?Object.fromEntries(Object.entries(parsed.quantities as Record<string,unknown>).map(([key,value])=>[key,String(value)])):{};const commercial=typeof parsed.commercial_order_lines==="object"&&parsed.commercial_order_lines?Object.fromEntries(Object.entries(parsed.commercial_order_lines as Record<string,unknown>).map(([key,value])=>[key,String(value)])):{};return{operation,lot_ids:parsed.lot_ids.map(String).filter(Boolean),quantities,commercial_order_lines:commercial,fulfillment_reference:String(parsed.fulfillment_reference??""),order_number:String(parsed.order_number??""),package_label:String(parsed.package_label??""),source:String(parsed.source??"")}}catch{return emptyStaged()}}
function emptyStaged():StagedTransferDraft{return{lot_ids:[],quantities:{},commercial_order_lines:{},fulfillment_reference:"",order_number:"",package_label:"",source:""}}
