...
export function InventoryTransferManager({operation,packages,onClose,onSaved}:{operation:Operation;packages:InventoryPackage[];onClose:()=>void;onSaved:(message:string)=>void}) {
  ...
  return <StreamlitDialog open onClose={onClose} eyebrow="INVENTORY · LICENSE TRANSFERS" title="Cross-license transfers" subtitle="Physical ledger movements stay separate by license while genealogy follows the material across facilities.">
    ...
    <section className="inventory-panel"><div className="eyebrow">OUTBOUND HISTORY</div><h3>Transfers sent from this license</h3>{outbound.length===0&&!transfers.isLoading?<div className="empty">No outbound DoobieLogic transfers are recorded for this facility.</div>:null}{outbound.map(row=><article className="plant-event" key={row.id}><strong>{row.manifest_reference} · {title(row.status)}</strong><span>{row.source_facility_name} → {row.destination_facility_name} ({row.destination_license_number||"license not recorded"})</span><small>{new Date(row.shipped_at).toLocaleString()} · {row.lines.reduce((sum,line)=>sum+line.quantity,0).toLocaleString(undefined,{maximumFractionDigits:4})} total units across {row.lines.length} line(s)</small>{row.status==="shipped"&&canWrite?<div className="audit-actions"><input placeholder="Cancellation reason" value={cancelReason} onChange={event=>setCancelReason(event.target.value)} /><button className="secondary" disabled={cancel.isPending} onClick={()=>cancel.mutate(row.id)}>Cancel before receipt</button></div>:null}</article>)}</section>
