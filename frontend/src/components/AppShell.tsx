import { BarChart3, BookOpen, Boxes, ClipboardCheck, Combine, Factory, FileText, FlaskConical, Gauge, Menu, Moon, PackageOpen, Plug, Scale, Settings, ShieldCheck, ShoppingCart, Sparkles, Sun, Tags, Truck, Users } from "lucide-react";
...
/* Streamlit destinations remain explicit. Newer ledger/ERP surfaces are kept
   too, but they are not allowed to replace an old Buyer Dash workspace. */
const navigation: NavigationItem[] = [
  ...
  { icon: ClipboardCheck, label: "Inventory Audits", page: "Inventory", capability: "retail" },
...
export function AppShell({ children, active, onNavigate }: PropsWithChildren<{ active: string; onNavigate: (page: string) => void }>) {
  ...
  const switchOrganization = (organizationId: string) => {
    ...
    client.clear(); window.location.reload();
  };
  ...
  const signOut = async () => {
    localStorage.removeItem("buyer-dash-organization"); localStorage.removeItem("buyer-dash-facility"); client.clear(); await supabase?.auth.signOut();
  };
  ...
  return <div className="app-shell">
    ...
    <aside className={navigationOpen ? "sidebar open" : "sidebar"} id="primary-navigation" aria-label="Primary navigation"><div className="brand"><span>BD</span><strong>Buyer Dash</strong></div><nav>{navigation.map((item,index) => {
