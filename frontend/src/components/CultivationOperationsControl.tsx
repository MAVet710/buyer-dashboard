...
import { HarvestOutputAllocation } from "./HarvestOutputAllocation";
import { MetrcHarvestControls, useMetrcHarvestStatus } from "./MetrcHarvestControls";
...
export function CultivationOperationsControl({plants,canWrite,onSelectPlant}:Props) {
  ...
  return <section className="cultivation-operations-control">
    <div className="section-heading"><div><div className="eyebrow">CULTIVATION OPERATIONS</div><h3>Rooms, Harvests & Cost</h3><p className="source-caption">Plan and run the physical cultivation operation in DoobieLogic. When a promoted Metrc environment is available, regulated checkpoints appear inside Harvest 360 and provider state is verified before local state changes.</p></div><div className="audit-actions">{canWrite?<><button className="secondary" type="button" onClick={()=>setRoomEditor("new")}>Add room</button><button className="primary" type="button" onClick={()=>setHarvestBuilder(true)}>Plan harvest</button></>:null}</div></div>
    <div className="metrics four"><div className="metric"><span>Active rooms</span><strong>{activeRooms.length}</strong></div><div className="metric"><span>Open harvests</span><strong>{openHarvests.length}</strong></div><div className="metric"><span>Capacity alerts</span><strong>{overCapacity}</strong></div><div className="metric"><span>Harvest COGS</span><strong>{money(harvestValue)}</strong></div></div>
    ...
    <div className="info-banner"><strong>Regulatory boundary:</strong> Harvest planning remains DoobieLogic operational state. Promoted Metrc harvest checkpoints execute only from Harvest 360 with confirmation, fresh readback, reconciliation, and durable evidence.</div>
...
function HarvestWorkspace({harvestId,canWrite,onSelectPlant,onChanged}:{harvestId:string;canWrite:boolean;onSelectPlant:(plant:CultivationPlant)=>void;onChanged:()=>void|Promise<void>}) {
  const client=useQueryClient();
  const metrcStatus=useMetrcHarvestStatus();
  const governed=Boolean(metrcStatus.data?.ready);
