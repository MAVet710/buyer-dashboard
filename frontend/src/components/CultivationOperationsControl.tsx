...
type Harvest = {
  id:string; harvest_code:string; strain_name:string; room_code:string; status:string; planned_date:string|null;
  started_at:string|null; finished_at:string|null; plant_count:number; wet_weight:number; dry_weight:number;
  waste_weight:number; unit:string; dry_yield_pct:number|null; labor_cost_usd:number; material_cost_usd:number;
...
type Props = {
  plants:CultivationPlant[];
  canWrite:boolean;
  onSelectPlant:(plant:CultivationPlant)=>void;
};
...
export function CultivationOperationsControl({plants,canWrite,onSelectPlant}:Props) {
  ...
  const openHarvests=(harvests.data?.items??[]).filter(row=>!["finished","cancelled"].includes(row.status));
  ...
  return <section className="cultivation-operations-control">
    ...
    <div className="two-col">
      ...
      <section className="inventory-panel"><div className="section-heading"><div><h4>Harvest Queue</h4><p className="source-caption">Plan flowering plants into a durable harvest, then track wet weight, dry weight, waste, yield and COGS in Harvest 360.</p></div></div>
        ...
        {harvests.data?.items.length?<div className="table-wrap"><table><thead><tr><th>Harvest</th><th>Status</th><th>Plants</th><th>Yield</th><th>COGS</th></tr></thead><tbody>{harvests.data.items.slice(0,30).map(row=><tr key={row.id} onClick={()=>setSelectedHarvestId(row.id)}><td><strong>{row.harvest_code}</strong><br/><small>{row.strain_name} · {row.room_code||"Mixed"}</small></td><td><span className="badge">{title(row.status)}</span><br/><small>{row.planned_date||"No plan date"}</small></td><td>{row.plant_count}</td><td>{row.dry_yield_pct==null?"—":`${row.dry_yield_pct.toFixed(1)}%`}<br/><small>{number(row.dry_weight)} {row.unit} dry</small></td><td>{money(row.total_cogs_usd)}{row.cost_per_dry_unit==null?null:<><br/><small>{money(row.cost_per_dry_unit)}/{row.unit}</small></>}</td></tr>)}</tbody></table></div>:harvests.data?<div className="empty">No harvests have been planned yet.</div>:null}
      </section>
    </div>

    <div className="info-banner"><strong>Regulatory boundary:</strong> starting or finishing a Harvest 360 record changes DoobieLogic operational state only. Metrc plant/harvest writes remain separately fail-closed and are not enabled by this workspace.</div>
...
function HarvestBuilder({plants,onClose,onSaved}:{plants:CultivationPlant[];onClose:()=>void;onSaved:(row:HarvestDetail)=>void}) {
  ...
  const [harvestCode,setHarvestCode]=useState("");const [plannedDate,setPlannedDate]=useState("");const [notes,setNotes]=useState("");const [selected,setSelected]=useState<string[]>([]);
  const create=useMutation({mutationFn:()=>apiPost<HarvestDetail>("/api/v1/inventory/production/plants/harvests",{harvest_code:harvestCode.trim(),plant_ids:selected,planned_date:plannedDate||null,notes:notes.trim()}),onSuccess:onSaved});
  const toggle=(id:string)=>setSelected(current=>current.includes(id)?current.filter(value=>value!==id):[...current,id]);
  return <StreamlitDialog open onClose={onClose} eyebrow="Cultivation" title="Plan harvest" subtitle="Assign flowering plants to one operational harvest. This does not submit a Metrc harvest."><div className="form-grid two"><label>Harvest code<input value={harvestCode} onChange={e=>setHarvestCode(e.target.value)} placeholder="HARV-2026-001"/></label><label>Planned date<input type="date" value={plannedDate} onChange={e=>setPlannedDate(e.target.value)}/></label><label className="span-2">Notes<textarea rows={2} value={notes} onChange={e=>setNotes(e.target.value)}/></label></div><div className="table-wrap"><table><thead><tr><th></th><th>Plant</th><th>Strain</th><th>Room</th><th>Est. harvest</th></tr></thead><tbody>{flowering.map(plant=><tr key={plant.id} onClick={()=>toggle(plant.id)}><td><input type="checkbox" checked={selected.includes(plant.id)} onChange={()=>toggle(plant.id)} onClick={e=>e.stopPropagation()}/></td><td><strong>{plant.plant_tag}</strong></td><td>{plant.strain_name}</td><td>{plant.room_code}</td><td>{plant.estimated_harvest_date||"—"}</td></tr>)}</tbody></table>{!flowering.length?<div className="empty">No flowering plants are available to assign.</div>:null}</div><div className="audit-actions"><span className="source-caption">{selected.length} selected</span><button className="primary" disabled={!harvestCode.trim()||!selected.length||create.isPending} onClick={()=>create.mutate()}>{create.isPending?"Creating…":"Create harvest"}</button></div>{create.isError?<div className="form-error">{create.error.message}</div>:null}</StreamlitDialog>;
}
...
function HarvestWorkspace({harvestId,canWrite,onSelectPlant,onChanged}:{harvestId:string;canWrite:boolean;onSelectPlant:(plant:CultivationPlant)=>void;onChanged:()=>void|Promise<void>}) {
  ...
  const transition=useMutation({mutationFn:(status:string)=>apiPost<HarvestDetail>(`/api/v1/inventory/production/plants/harvests/${harvestId}/transition`,{status,wet_weight:numOrNull(weights.wet_weight),dry_weight:numOrNull(weights.dry_weight),waste_weight:numOrNull(weights.waste_weight),unit:weights.unit.trim(),notes:weights.notes.trim()}),onSuccess:refresh});
  ...
  return <div className="harvest-360-workspace">
    ...
    <div className="two-col"><section className="inventory-panel"><h3>Assigned plants</h3><div className="table-wrap"><table><thead><tr><th>Tag</th><th>Strain</th><th>Room</th><th>Phase</th></tr></thead><tbody>{row.plants.map(plant=><tr key={plant.id} onClick={()=>onSelectPlant(plant)}><td><strong>{plant.plant_tag}</strong></td><td>{plant.strain_name}</td><td>{plant.room_code}</td><td>{title(plant.phase)}</td></tr>)}</tbody></table></div></section><section className="inventory-panel"><h3>Yield & loss</h3><div className="metrics"><Metric label="Wet" value={`${number(row.wet_weight)} ${row.unit}`}/><Metric label="Dry" value={`${number(row.dry_weight)} ${row.unit}`}/><Metric label="Waste" value={`${number(row.waste_weight)} ${row.unit}`}/></div>{canWrite&&!['finished','cancelled'].includes(row.status)?<><div className="form-grid two"><label>Wet weight<input type="number" min="0" value={weights.wet_weight} onChange={e=>setWeights({...weights,wet_weight:e.target.value})}/></label><label>Dry weight<input type="number" min="0" value={weights.dry_weight} onChange={e=>setWeights({...weights,dry_weight:e.target.value})}/></label><label>Waste weight<input type="number" min="0" value={weights.waste_weight} onChange={e=>setWeights({...weights,waste_weight:e.target.value})}/></label><label>Unit<input value={weights.unit} onChange={e=>setWeights({...weights,unit:e.target.value})}/></label><label className="span-2">Execution notes<input value={weights.notes} onChange={e=>setWeights({...weights,notes:e.target.value})}/></label></div><div className="audit-actions">{next.map(status=><button key={status} className={status==="cancelled"?"secondary":"primary"} disabled={transition.isPending} onClick={()=>transition.mutate(status)}>{transition.isPending?"Saving…":harvestAction(status)}</button>)}</div>{transition.isError?<div className="form-error">{transition.error.message}</div>:null}</>:null}</section></div>
