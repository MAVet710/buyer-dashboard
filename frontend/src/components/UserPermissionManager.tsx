...
type Permission={key:string;group:string;label:string;description:string;coverage?:string};
...
export function UserPermissionManager(){
  ...
  const groups=useMemo(()=>Array.from(new Set((registry.data??[]).map(permission=>permission.group))),[registry.data]);
  ...
  return <details className="streamlit-expander admin-user-permissions" open>
    ...
    <div className="streamlit-expander-body">
      <p>Roles provide safe defaults. Use facility-specific overrides only for exceptions. An explicit deny overrides the role default; inherit returns the user to their role default. Allow does not bypass existing role, facility, or provider checks. Coverage is limited to the actions described below. Level DEV retains platform access.</p>
      {!manageable.length?<div className="info-banner">No non-DEV users are available for permission overrides.</div>:<>
        ...
        {registry.isError?<div className="state error">Unable to load the permission registry: {registry.error.message}</div>:null}
