...
type CatalogOption = { product_id:string;sku:string;name:string;unit:string;available:number;on_hand:number;suggested_price_usd:number;eligible:boolean;blocked_reasons:string[] };
...
export function CommerceStorefrontManager() {
  ...
  const allListingsSelected=listings.length>0&&listings.every(row=>row.selected);
