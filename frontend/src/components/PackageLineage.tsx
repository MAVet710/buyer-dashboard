...
export function PackageLineage({ operation, item, onClose }: { operation: "retail" | "production"; item: InventoryPackage; onClose: () => void }) {
  ...
  return <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-label="Package lineage">
    <div className="modal-heading"><div><div className="eyebrow">Package 360</div><h2>{item.product_name}</h2><p>{item.package_id} · {item.available.toLocaleString()} {item.unit}</p></div><button className="secondary" onClick={onClose}>Close</button></div>
