...
export function PackageLineage({ operation, item, onClose }: { operation: "retail" | "production"; item: InventoryPackage; onClose: () => void }) {
  ...
  return <StreamlitDialog open onClose={onClose} eyebrow="Package 360" title={item.product_name} subtitle={`${item.package_id} · ${item.available.toLocaleString()} ${item.unit}`}>
    {trail.isLoading ? <div className="state">Loading source trail…</div> : null}
    {trail.error ? <div className="state error">{trail.error.message}</div> : null}
    {trail.data ? <div className="lineage">
      ...
      <div className="lineage-current"><strong>{trail.data.lot.product_name}</strong><span>{trail.data.lot.lot_code}</span><b>{trail.data.lot.balance} {trail.data.lot.unit}</b></div>
      <section><h3>Used by</h3>{trail.data.used_by.map(run => <article key={`${run.run_number}-${run.quantity_consumed}`}><strong>{run.run_number} · {run.action_type}</strong><span>Consumed {run.quantity_consumed} {run.unit}</span>{run.outputs.map(output => <small key={output.lot_code}>→ {output.lot_code} · {output.inventory_quantity} {output.inventory_unit}</small>)}</article>)}{trail.data.used_by.length === 0 ? <p>Not consumed by a later transformation.</p> : null}</section>
    </div> : null}
  </StreamlitDialog>;
}
