...
function ProviderCard({ provider, value }: { provider: ProviderKey; value: Connection }) {
  ...
  return <article className="integration-card">
    ...
    {provider === "metrc" ? <p className="source-caption">1. Save the Metrc Connect integrator/vendor key. 2. Provision the generic sandbox user. 3. Metrc sends the separate sandbox User API Key to the integrator contact email. 4. Save that User API Key in the main METRC integration card and validate facilities.</p> : null}
    ...
    <div className="connection-result">
      ...
      <p>Resources: {value.sandbox_resources.join(", ")}. Provider credentials stay encrypted server-side. Normal authenticated Metrc reads use the distinct integrator + user key pair after sandbox user provisioning.</p>
