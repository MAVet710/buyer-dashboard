...
export function InventoryDrivenLabelWorkflow(){
  const inventory=useQuery({queryKey:["label-studio-inventory-summaries"],queryFn:({signal})=>apiGet<InventorySummary[]>("/api/v1/label-printing/inventory-sources?summary=true",signal)});
  ...
  const selectedSummary=inventory.data?.find(row=>row.lot_id===sourceLotId);
  ...
  return <section className="inventory-panel production-label-workflow">
    ...
    <div className="production-label-grid">
      <label>1. Source batch<select aria-label="Source batch" value={sourceLotId} disabled={Boolean(run)} onChange={event=>setSourceLotId(event.target.value)}><option value="">Select tested inventory…</option>{inventory.data?.map(row=><option key={row.lot_id} value={row.lot_id}>{sourceName(row)} · {row.on_hand:g}{row.inventory_unit}</option>)}</select></label>
      <label>2. End product<select aria-label="End product" value={productId} disabled={Boolean(run)} onChange={event=>setProductId(event.target.value)}><option value="">Select finished product…</option>{productRows.map(row=><option key={row.id} value={row.id}>{productName(row)}</option>)}</select></label>
    ...
    {sourceLotId?<div className="catalog-row"><strong>{source.isLoading?"Loading source evidence…":sourceReady?"✓ Verified source":"Source needs attention"}</strong><span>{selectedSummary?`${selectedSummary.on_hand} ${selectedSummary.inventory_unit} available`:""}</span><small>{source.data?.coa.available?`${source.data.coa.lab_name||"Lab"} · tested ${source.data.coa.date_tested||"date missing"} · ${source.data.coa.overall_status||"status missing"}`:"No verified COA resolved"}</small></div>:null}
    {productId?<div className="catalog-row"><strong>{product.isLoading?"Loading package specifications…":packaging?"✓ Product package configured":"Package setup required"}</strong><span>{packaging?`${packaging.units_per_package:g} units/package · ${packaging.net_content:g} ${packaging.net_content_unit}`:""}</span><small>{packaging&&expected>0?`${quantity} finished packages = ${expected:g} ${packaging.net_content_unit} theoretical material before loss/waste`:"Configure package facts in Product Master"}</small></div>:null}
