...
export function InventoryDrivenLabelWorkflow(){
  ...
  const selectedSummary=inventoryRows.find(row=>row.lot_id===sourceLotId);
  ...
  return <section className="inventory-panel production-label-workflow">
    ...
    <div className="production-label-grid">
      <label>1. Source batch<select aria-label="Source batch" value={sourceLotId} disabled={Boolean(run)} onChange={event=>setSourceLotId(event.target.value)}><option value="">Select tested inventory…</option>{inventoryRows.map(row=><option key={row.lot_id} value={row.lot_id}>{sourceName(row)} · {fmt(row.on_hand)} {row.inventory_unit}</option>)}</select></label>
