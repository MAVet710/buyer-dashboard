...
type LabelRun = {
  ...
  created_at:string;printed_at:string|null;snapshot:{source:SourceSnapshot;sources?:SourceSnapshot[];product:Record<string,unknown>;label:Record<string,string>;quantity:number;print_layout?:PrintLayout;expected_material_quantity:number;expected_material_unit:string};
...
export function InventoryDrivenLabelWorkflow(){
  ...
  const primaryReady=sourceReady(source.data);
  const secondReady=!needsSecondSource||sourceReady(secondarySource.data);
  ...
  return <section className="inventory-panel production-label-workflow">
    ...
    {sourceLotId?<div className="catalog-row"><strong>{source.isLoading?"Loading source evidence…":primaryReady?"✓ Verified source":"Source needs attention"}</strong><small>{source.data?.coa.available?`${source.data.coa.lab_name||"Lab"} · tested ${source.data.coa.date_tested||"date missing"} · ${source.data.coa.overall_status||"status missing"}`:"No verified COA resolved"}</small></div>:null}
    {needsSecondSource&&secondarySourceLotId?<div className="catalog-row"><strong>{secondarySource.isLoading?"Loading second source…":secondReady?"✓ Second source verified":"Second source needs attention"}</strong><small>{secondarySource.data?.coa.available?`${secondarySource.data.coa.lab_name||"Lab"} · tested ${secondarySource.data.coa.date_tested||"date missing"} · ${secondarySource.data.coa.overall_status||"status missing"}`:"No verified COA resolved"}</small></div>:null}
    ...
    {run?<div className="production-label-preview"><div className="eyebrow">4. GENERATED LABEL PREVIEW · {run.status.toUpperCase()}</div><h3>{label.product_name||"Finished product"}</h3><div className="production-label-preview-grid"><div><span>Print layout</span><strong>{layoutName(printLayout.layout)}</strong></div><div><span>Label stock</span><strong>{printLayout.width_in} × {printLayout.height_in} in</strong></div><div><span>Tested sources</span><strong>{sources.length}</strong></div>{DISPLAY_FIELDS.filter(field=>String(label[field]??"").trim()).map(field=><div key={field}><span>{fieldLabel(field)}</span><strong>{label[field]}</strong></div>)}{sources.map((item,index)=><div key={item.lot_id}><span>{sources.length>1?`Source ${index+1}`:"Source package"}</span><strong>{item.package_id||item.lot_code}</strong></div>)}<div><span>Finished quantity</span><strong>{run.quantity} labels</strong></div></div>
