...
import { labelIndices, labelReprintBlock, validLabelRange } from "../lib/labelReprints";
...
type WorkflowProps = { sandboxTestPass?:boolean; restoredRun?:LabelRun; canWrite?:boolean; onStartNew?:()=>void; initialSourceLotId?:string };
...
function eventLabel(value:string){if(value==="printed")return "Print requested";if(value==="reprinted")return "Reprint requested";return value.replaceAll("_"," ").replace(/\b\w/g,match=>match.toUpperCase());}
...
function CompactSingleLabel({run,index,sources}:{run:LabelRun;index:number;sources:SourceSnapshot[]}){
  ...
  return <>
    ...
    <div className="printed-two-column"><AnalyteList title="Cannabinoids" rows={analytes(source,"cannabinoids",8)} total={source?.coa.total_cannabinoids}/><AnalyteList title="Terpenes" rows={analytes(source,"terpenes",6)} total={source?.coa.total_terpenes}/></div>
...
export function InventoryDrivenLabelWorkflow({sandboxTestPass=false,restoredRun,canWrite=true,onStartNew,initialSourceLotId=""}:WorkflowProps){
  const client=useQueryClient();
  const historical=Boolean(restoredRun);
  const printRoot=useRef<HTMLDivElement>(null);
  const mounted=useRef(true);
  useEffect(()=>{mounted.current=true;return()=>{mounted.current=false;};},[]);
  // A historical snapshot must not depend on current stock, Product Master, or COA availability.
  const inventory=useQuery({queryKey:["label-studio-inventory-summaries"],queryFn:({signal})=>apiGet<unknown>("/api/v1/label-printing/inventory-sources?summary=true",signal),enabled:!historical});
  const products=useQuery({queryKey:["label-studio-finished-products"],queryFn:({signal})=>apiGet<unknown>("/api/v1/product-master?operation=production&search=&status=active&item_type=finished_good",signal),enabled:!historical});
  ...
  const [sourceLotId,setSourceLotId]=useState(initialSourceLotId);
  const [secondarySourceLotId,setSecondarySourceLotId]=useState("");
  const [productId,setProductId]=useState("");
  const [quantity,setQuantity]=useState(1);
  const [tag,setTag]=useState("");
  const [run,setRun]=useState<LabelRun|null>(restoredRun??null);
  const [reprintReason,setReprintReason]=useState("");
  const [reprintCopies,setReprintCopies]=useState(1);
  const [firstLabel,setFirstLabel]=useState(1);
  const [printRange,setPrintRange]=useState({copies:1,firstLabel:1});
  ...
  const source=useQuery({queryKey:["label-studio-production-source",sourceLotId],queryFn:({signal})=>apiGet<InventorySource>(`/api/v1/label-printing/inventory-sources/${encodeURIComponent(sourceLotId)}`,signal),enabled:Boolean(sourceLotId)&&!historical});
  const secondarySource=useQuery({queryKey:["label-studio-production-source",secondarySourceLotId],queryFn:({signal})=>apiGet<InventorySource>(`/api/v1/label-printing/inventory-sources/${encodeURIComponent(secondarySourceLotId)}`,signal),enabled:Boolean(secondarySourceLotId)&&!historical});
  const product=useQuery({queryKey:["label-studio-production-product",productId],queryFn:({signal})=>apiGet<ProductDetail>(`/api/v1/product-master/${encodeURIComponent(productId)}`,signal),enabled:Boolean(productId)&&!historical});
  ...
  const create=useMutation({mutationFn:()=>apiPost<LabelRun>("/api/v1/label-printing/production-runs",{source_lot_id:sourceLotId,...(needsSecondSource?{secondary_source_lot_id:secondarySourceLotId}:{}),product_id:productId,quantity}),onSuccess:value=>{setRun(value);setPrintRange({copies:value.quantity,firstLabel:1});void client.invalidateQueries({queryKey:["label-run-history"]});}});
  ...
  const print=useMutation({
    mutationFn:(range:{copies:number;firstLabel:number})=>{
      if(!run||!canWrite||!validLabelRange(run.quantity,range.copies,range.firstLabel))throw new Error("Choose a valid original label range before printing.");
      const blocked=labelReprintBlock(run.status);if(blocked)throw new Error(blocked);
      if(run.status!=="tagged"&&!reprintReason.trim())throw new Error("A reprint reason is required.");
      const reason=run.status==="tagged"?"":`${reprintReason.trim()} [Original labels ${range.firstLabel}–${range.firstLabel+range.copies-1} of ${run.quantity}]`;
      return apiPost<LabelRun>(`/api/v1/label-printing/production-runs/${run.id}/print`,{copies:range.copies,reason});
    },
    onSuccess:async(value,range)=>{
      void client.invalidateQueries({queryKey:["label-run-history"]});
      if(!mounted.current)return;
      setRun(value);setPrintRange(range);setReprintReason("");setPrintNotice("Print request saved. The browser cannot confirm physical printer output; a cancelled dialog still leaves this request in the audit trail.");
      await new Promise<void>(resolve=>window.requestAnimationFrame(()=>window.requestAnimationFrame(()=>resolve())));
      const root=printRoot.current;
      if(!mounted.current||!root||root.dataset.runId!==value.id)return;
      try {
        if(document.fonts)await document.fonts.ready;
        await Promise.all(Array.from(root.querySelectorAll("img")).map(image=>image.decode()));
        if(mounted.current&&printRoot.current===root)window.print();
      } catch {
        if(mounted.current)setPrintNotice("The print request was saved, but the preview or print dialog could not finish loading. No physical output was confirmed. Reopen this saved run and reprint with a reason.");
      }
    },
  });
  ...
  const reset=()=>{if(historical){onStartNew?.();return;}setSourceLotId("");setSecondarySourceLotId("");setProductId("");setQuantity(1);setTag("");setRun(null);setReprintReason("");setReprintCopies(1);setFirstLabel(1);setPrintRange({copies:1,firstLabel:1});setPrintNotice("");setDraftDesign(null);setOverflow([]);saveDesign.reset();create.reset();assign.reset();print.reset();transition.reset()};
  const error=saveDesign.error||create.error||assign.error||print.error||transition.error||source.error||secondarySource.error||product.error||inventory.error||products.error;
  ...
  const snapshotReady=Boolean(run&&sources.length&&sources.every(item=>item?.label&&item?.coa));
  const label=run?.snapshot?.label??{};
  const copies=run?labelIndices(run.quantity,printRange.copies,printRange.firstLabel):[];
  ...
  const contents=useMemo(()=>run&&snapshotReady?customContents(run):null,[run,snapshotReady]);
  const layoutProblems=draftDesign?designIssues(draftDesign):customDesign?designIssues(customDesign):[];
  const missingSavedLayout=historical&&!run?.snapshot.print_layout&&!customDesign;
  const printBlocked=!canWrite||!snapshotReady||missingSavedLayout||Boolean(draftDesign)||Boolean(customDesign&&(overflow.length||layoutProblems.length));
  const rangeValid=Boolean(run&&validLabelRange(run.quantity,reprintCopies,firstLabel));
  ...
  const requestPrint=()=>{if(!run||printBlocked)return;const range=run.status==="tagged"?{copies:run.quantity,firstLabel:1}:{copies:reprintCopies,firstLabel};if(!validLabelRange(run.quantity,range.copies,range.firstLabel))return;setPrintNotice("");setPrintRange(range);print.mutate(range);};
  ...
  return <section className="inventory-panel production-label-workflow">
    ...
    `}</style>
    {sandboxTestPass&&!historical?<div className="sandbox-test-pass" role="status"><strong>DEV SANDBOX · ALL OPERATIONAL DATA IS TEST DATA · PRINT TEST PASS ACTIVE</strong><small>This pass is restricted to the DEV role inside dev-sandbox / SANDBOX. Missing or non-passing COA readiness and sandbox METRC tag availability can be bypassed for print testing. Product/facility scope, tag format, local tag uniqueness, and production safeguards remain enforced.</small></div>:null}
    <div className="eyebrow">LABEL STUDIO · {historical?"SAVED LABEL RUN":"CREATE FROM INVENTORY"}</div>
    <div className="page-heading"><div><h2>{historical?"Saved label & reprints":"Create the finished label"}</h2><p>{historical?"This is the original saved label snapshot. Reprinting does not create a package, assign another tag, or change inventory.":"Search the tested source package, choose the finished product, verify its saved label layout and stock size, scan the finished METRC package tag, and print."}</p></div>{run&&(!historical||onStartNew)?<button className="secondary" disabled={print.isPending} onClick={reset}>Start new label run</button>:null}</div>
    {!canWrite?<div className="info-banner">Your role can view saved labels but cannot create, print, or change label runs.</div>:null}
    {run?.status==="archived"?<div className="info-banner">{labelReprintBlock("archived")}</div>:null}
    {missingSavedLayout||run&&!snapshotReady?<div className="warning-banner">The original label snapshot is incomplete. Reprinting is disabled rather than substituting current product or testing information.</div>:null}
    {printNotice?<div className="info-banner" role="status">{printNotice}</div>:null}
    ...
    {!historical?<>
      <div className="production-label-grid">
        <label>1. Source batch<SearchablePicker ariaLabel="Search source batches" value={sourceLotId} options={sourceOptions} placeholder="Search strain, batch, SKU, or METRC package…" disabled={Boolean(run)||!canWrite} onChange={setSourceLotId}/></label>
        <label>2. End product<SearchablePicker ariaLabel="Search finished products" value={productId} options={productOptions} placeholder="Search product name, SKU, brand, or format…" disabled={Boolean(run)||!canWrite} onChange={setProductId}/></label>
        <label>3. Finished quantity<input aria-label="Finished quantity" type="number" min="1" max="500" step="1" value={quantity} disabled={Boolean(run)||!canWrite} onChange={event=>setQuantity(Math.max(1,Math.min(500,Number(event.target.value)||1)))}/></label>
        {needsSecondSource?<label>Second tested source<SearchablePicker ariaLabel="Search second source batch" value={secondarySourceLotId} options={sourceOptions.filter(option=>option.value!==sourceLotId)} placeholder="Search second strain or METRC package…" disabled={Boolean(run)||!canWrite} onChange={setSecondarySourceLotId}/></label>:null}
      </div>
    ...
    {run?<div className="production-label-preview"><div className="eyebrow">4. GENERATED LABEL PREVIEW · {run.status.toUpperCase()}</div><h3>{label.product_name||"Finished product"}</h3>{run.snapshot.sandbox?.sandbox_test_pass?<div className="sandbox-test-pass"><strong>Sandbox test-pass recorded on this run</strong><small>{run.snapshot.sandbox.bypassed_checks?.length?`Bypassed: ${run.snapshot.sandbox.bypassed_checks.join(", ")}`:"No source-readiness checks needed bypassing."}</small></div>:null}<div className="production-label-preview-grid"><div><span>Print layout</span><strong>{layoutName(printLayout.layout)}</strong></div><div><span>Label stock</span><strong>{printLayout.width_in} × {printLayout.height_in} in</strong></div><div><span>Tested sources</span><strong>{sources.length}</strong></div>{DISPLAY_FIELDS.filter(field=>String(label[field]??"").trim()).map(field=><div key={field}><span>{fieldLabel(field)}</span><strong>{label[field]}</strong></div>)}{sources.map((item,index)=><div key={item.lot_id}><span>{sources.length>1?`Source ${index+1}`:"Source package"}</span><strong>{item.package_id||item.lot_code}</strong></div>)}<div><span>Finished quantity</span><strong>{run.quantity} labels</strong></div></div>
      {contents?<div style={{marginTop:16}}>
        {canWrite&&!draftDesign&&["validated","tagged"].includes(run.status)?<button className="secondary" disabled={assign.isPending||print.isPending} onClick={()=>{setOverflow([]);setDraftDesign(customDesign??initialDesign(printLayout.width_in,printLayout.height_in));}}>Customize size & layout</button>:null}
        {draftDesign?<LabelLayoutEditor design={draftDesign} contents={contents} disabled={!canWrite||!["validated","tagged"].includes(run.status)} busy={saveDesign.isPending} onChange={setDraftDesign} onSave={()=>saveDesign.mutate()} onCancel={()=>{setDraftDesign(null);setOverflow([]);saveDesign.reset();}} onOverflow={setOverflow}/>:customDesign?<div className="label-design-readonly"><LabelDesignCanvas design={customDesign} contents={contents} onOverflow={setOverflow}/></div>:null}
      ...
      {canWrite&&run.status==="tagged"?<div style={{marginTop:16}}><button className="primary" disabled={print.isPending||saveDesign.isPending||printBlocked} onClick={requestPrint}>{print.isPending?"Recording print…":`6. Finalize & print ${run.quantity} labels`}</button></div>:null}
      {canWrite&&["printed","applied","released","fulfilled"].includes(run.status)?<div style={{marginTop:16}}>
        <div className="form-grid"><label>Replacement copies<input aria-label="Replacement copies" type="number" min="1" max={run.quantity} step="1" value={reprintCopies} disabled={print.isPending} onChange={event=>setReprintCopies(Number(event.target.value))}/></label><label>First original label number<input aria-label="First original label number" type="number" min="1" max={run.quantity} step="1" value={firstLabel} disabled={print.isPending} onChange={event=>setFirstLabel(Number(event.target.value))}/></label><label>Reprint reason<input aria-label="Reprint reason" maxLength={400} placeholder="Reason required for reprint" value={reprintReason} disabled={print.isPending} onChange={event=>setReprintReason(event.target.value)}/></label></div>
        {!rangeValid?<div className="form-error">Choose a whole-number replacement range within the original {run.quantity} labels.</div>:null}
        <div className="inline-form"><button className="secondary" disabled={!reprintReason.trim()||!rangeValid||print.isPending||printBlocked} onClick={requestPrint}>Reprint {reprintCopies} replacement{reprintCopies===1?"":"s"}</button>{nextStatus?<button className="primary" disabled={transition.isPending||print.isPending} onClick={()=>transition.mutate(nextStatus)}>{nextAction}</button>:null}</div><p className="section-note">Original batch quantity, tag, layout and testing data remain unchanged. Replacement numbering uses the selected original range.</p>
      </div>:null}
    </div>:null}
    {run?.metrc_package_tag&&snapshotReady&&!missingSavedLayout?<div ref={printRoot} data-run-id={run.id} className="production-label-print-batch" aria-label="Printable retail labels">{copies.map(index=><div className={`production-label-copy layout-${printLayout.layout}${customDesign?" has-custom-design":""}`} key={index} data-layout={printLayout.layout} data-label-width={printLayout.width_in} data-label-height={printLayout.height_in}>{customDesign?<LabelDesignCanvas design={customDesign} contents={customContents(run,index)}/>:printLayout.layout==="compact_split"?<CompactSplitLabel run={run} index={index} sources={sources}/>:printLayout.layout==="bulk_barcode"?<BulkBarcodeLabel run={run} index={index} sources={sources}/>:<CompactSingleLabel run={run} index={index} sources={sources}/>}</div>)}</div>:null}
    {run?<div style={{marginTop:18}}><h3>Audit trail</h3><p className="section-note">Print entries record requests. Browser printing does not report whether paper was successfully printed.</p>{run.events.map(event=><div className="label-audit-row" key={event.id}><strong>{eventLabel(event.event_type)}</strong><span>{event.from_status&&event.to_status?`${event.from_status} → ${event.to_status}`:event.to_status||event.from_status}{event.details.copies!=null?<><br/>{String(event.details.copies)} requested copies</>:null}{event.details.reason?<><br/>{String(event.details.reason)}</>:null}</span><small>{event.actor} · {new Date(event.occurred_at).toLocaleString()}</small></div>)}</div>:null}
