...
function metrcTagSuffix(value:string){return String(value??"").trim().slice(-5);}
...
function PrintedMetrcQr({run}:{run:LabelRun}){
  const tag=String(run.metrc_package_tag??"").trim();
  const qrValue=String(run.traceability.qr?.value??"").trim();
  if(!tag||qrValue!==tag)return null;
  const suffix=metrcTagSuffix(tag);
  return <div className="printed-metrc-qr" data-metrc-tag={tag}>
    <img src={graphicDataUri(run.traceability.qr)} alt={`QR code for finished METRC package ${tag}`}/>
    <span className="printed-metrc-tag-suffix" aria-label={`Last 5 of METRC tag ${suffix}`}>{suffix}</span>
  </div>;
}
...
function CompactSingleLabel({run,index,sources}:{run:LabelRun;index:number;sources:SourceSnapshot[]}){
  ...
  return <>
    ...
    <div className="printed-footer"><div><p>Cultivated by {sourceLabel.cultivated_by||"—"}</p><p>{sourceLabel.cultivator_license}</p><p>Manufactured by {label.manufacturer||sourceLabel.manufacturer||label.facility_name}</p><p>{label.license_number}</p></div><PrintedMetrcQr run={run}/></div>
...
function CompactSplitLabel({run,index,sources}:{run:LabelRun;index:number;sources:SourceSnapshot[]}){
  ...
  return <>
    ...
    <div className="printed-footer compact-split-footer"><div><p>{label.warning_text}</p></div><PrintedMetrcQr run={run}/></div>
    <div className="printed-unit-id">#{index+1} / {run.quantity}</div>
  </>;
}
...
export function InventoryDrivenLabelWorkflow({sandboxTestPass=false}:{sandboxTestPass?:boolean}){
  ...
  return <section className="inventory-panel production-label-workflow">
    ...
    {run?<div className="production-label-preview"><div className="eyebrow">4. GENERATED LABEL PREVIEW · {run.status.toUpperCase()}</div><h3>{label.product_name||"Finished product"}</h3>{run.snapshot.sandbox?.sandbox_test_pass?<div className="sandbox-test-pass"><strong>Sandbox test-pass recorded on this run</strong><small>{run.snapshot.sandbox.bypassed_checks?.length?`Bypassed: ${run.snapshot.sandbox.bypassed_checks.join(", ")}`:"No source-readiness checks needed bypassing."}</small></div>:null}<div className="production-label-preview-grid"><div><span>Print layout</span><strong>{layoutName(printLayout.layout)}</strong></div><div><span>Label stock</span><strong>{printLayout.width_in} × {printLayout.height_in} in</strong></div><div><span>Tested sources</span><strong>{sources.length}</strong></div>{DISPLAY_FIELDS.filter(field=>String(label[field]??"").trim()).map(field=><div key={field}><span>{fieldLabel(field)}</span><strong>{label[field]}</strong></div>)}{sources.map((item,index)=><div key={item.lot_id}><span>{sources.length>1?`Source ${index+1}`:"Source package"}</span><strong>{item.package_id||item.lot_code}</strong></div>)}<div><span>Finished quantity</span><strong>{run.quantity} labels</strong></div></div>
      ...
      {run.metrc_package_tag?<div className="production-traceability"><div className="production-traceability-qr"><img src={graphicDataUri(run.traceability.qr)} alt={`QR code for finished METRC package ${run.metrc_package_tag}`}/><strong aria-label={`Last 5 of METRC tag ${metrcTagSuffix(run.metrc_package_tag)}`}>{metrcTagSuffix(run.metrc_package_tag)}</strong></div><div><strong>{run.metrc_package_tag}</strong><img src={graphicDataUri(run.traceability.barcode)} alt={`Code 128 barcode for finished METRC package ${run.metrc_package_tag}`}/><p className="section-note">The QR encodes the exact finished-package METRC tag assigned in step 5. The saved Product Master stock size and layout will be used when the browser print dialog opens.</p></div></div>:null}
