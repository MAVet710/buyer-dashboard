...
export type AuditSummary = { id: string; audit_number: string; operation_type: "retail" | "production"; status: string; scope_label: string; blind_count: boolean; recount_tolerance: number; started_at: string; completed_at: string | null; created_by: string };
export type AuditLine = { id: string; lot_id: string; product_name: string; package_id: string; location: string; expected_quantity: number | null; counted_quantity: number | null; variance_quantity: number; recount_required: boolean; unit: string; reason: string; notes: string };
export type AuditDetail = { audit: AuditSummary; lines: AuditLine[] };
