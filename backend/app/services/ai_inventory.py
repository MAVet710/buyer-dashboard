...
class CurrentInventoryObservation:
        ...
        try:
            ...
            with Session(self.engine) as session:
                ...
                if not allowed:
                    ...
                    return
                # Bound hydration before calling the existing service. Refuse
                # an oversized read explicitly instead of returning a partial
    ...
    def evidence(self, access: DatasetAccessContext) -> pd.DataFrame:
        self._observe(access)
        return pd.DataFrame([dict(self._evidence)], columns=EVIDENCE_COLUMNS)
