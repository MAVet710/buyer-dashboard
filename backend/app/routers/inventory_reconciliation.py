...
def _ready_metrc(
    *,
    ...
    if not metrc.row or not metrc.user_api_key or not metrc.integrator_api_key or not metrc.state or not metrc.license_number:
        return normalized, metrc, False
    if metrc.status != "connected":
        raise HTTPException(
            status_code=409,
            detail="Validate the Metrc connection for this exact facility before reconciling live inventory.",
        )
    if not metrc.trusted_mapping:
        raise HTTPException(
            status_code=409,
            detail="An administrator must verify the exact Metrc facility, license, jurisdiction, credential, and environment mapping before reconciliation.",
        )
