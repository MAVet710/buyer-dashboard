...
def partners(context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    return [{key: getattr(row, key) for key in ("id", "name", "partner_type", "license_or_registration", "contact_name", "contact_email", "payment_terms", "active")} for row in _repo(engine).list_trade_partners(context.organization_id, active_only=False)]
...
def create_partner(payload: PartnerCreate, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    try:
        ...
        return {key: getattr(row, key) for key in ("id", "name", "partner_type", "license_or_registration", "contact_name", "contact_email", "payment_terms", "active")}
...
def order_detail(order_id: str, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    return {"order": {key: getattr(order, key) for key in ("id", "order_number", "order_type", "order_date", "due_at", "status", "payment_status", "currency", "notes")}, "lines": [{key: getattr(row, key) for key in ("id", "product_id", "description", "sku_snapshot", "quantity", "unit", "unit_price", "fulfilled_quantity", "notes")} for row in lines], "allocations": [{key: getattr(row, key) for key in ("id", "commercial_order_line_id", "lot_id", "quantity", "fulfilled_quantity", "status")} for row in allocations], "invoices": [{key: getattr(row, key) for key in ("id", "invoice_number", "status", "issue_date", "due_date", "total_usd", "balance_usd")} for row in finance["invoices"]], "shipments": [{key: getattr(row, key) for key in ("id", "shipment_number", "status", "manifest_reference", "carrier", "tracking_reference", "shipped_at", "delivered_at")} for row in finance["shipments"]]}
