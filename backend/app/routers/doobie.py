...
from ..services.regulatory_actions import RegulatoryActionProposalService, action_recommendations
...
class PackageFinishCreate(BaseModel):
    lot_id: str = Field(min_length=1, max_length=64)
    actual_date: str = Field(default="", max_length=32)
...
def _item(row):
    return {
        key: getattr(row, key)
        for key in (
            "id", "action_type", "title", "rationale", "financial_impact_usd",
            "risk_level", "status", "source_type", "source_id", "created_by",
...
def _proposal_by_id(service: DoobieActionService, context: RequestContext, proposal_id: str):
    return next(
        (
            row
            for row in service.list_proposals(
                context.organization_id,
                context.facility_id,
                statuses=("proposed", "approved", "executing", "executed", "failed", "rejected", "expired"),
                limit=500,
            )
...
def actions(context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    return {
        ...
        "items": [
            _item(row)
            for row in DoobieActionService(engine).list_proposals(
                context.organization_id,
                context.facility_id,
                statuses=("proposed", "approved", "executing", "executed", "rejected", "failed", "expired"),
            )
        ],
    }
...
def propose(payload: ProposalCreate, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    if payload.action_type in {"prepare_transfer_manifest", "prepare_regulatory_action"}:
        raise HTTPException(422, "Regulatory actions must be created through a governed deterministic Doobie builder.")
    try:
        return _item(
            DoobieActionService(engine).propose(
                organization_id=context.organization_id,
                facility_id=context.facility_id,
                actor=context.user_id,
                idempotency_key=payload.idempotency_key or f"web:{uuid4()}",
                source_type="web",
...
@router.get("/regulatory-actions/catalog")
def regulatory_action_catalog(
    ...
    return {
        ...
        "connected": metrc.status == "connected",
        ...
        "items": service.catalog(jurisdiction_code=metrc.state, environment=metrc.environment),
        "receiving_policy": {
            "operation_type": "inbound_transfer_accept",
            "provider_submission_enabled": False,
            "message": "Normal incoming-transfer acceptance remains fail-closed because the reviewed Massachusetts v2 API index does not expose a general accept-existing-inbound endpoint. Doobie can reconcile/read provider state but will not invent a write call.",
        },
    }


@router.get("/regulatory-actions/recommendations")
def regulatory_action_recommendations(
    context: RequestContext = Depends(get_request_context),
    engine: Engine = Depends(get_engine),
    settings: Settings = Depends(get_settings),
):
    intelligence = RegulatoryIntelligenceService(engine, settings).collect(context)
    scope = intelligence.get("scope") if isinstance(intelligence.get("scope"), dict) else {}
    catalog = RegulatoryActionProposalService.catalog(
        jurisdiction_code=str(scope.get("jurisdiction_code") or ""),
        environment=str(scope.get("environment") or ""),
    )
    return {
        "read_only_analysis": True,
        "provider_payloads_generated_by_model": False,
        "recommendations": action_recommendations(intelligence=intelligence, catalog=catalog),
        "catalog": catalog,
    }


@router.post("/regulatory-actions/package-finish", status_code=201)
def build_package_finish_action(
    payload: PackageFinishCreate,
    context: RequestContext = Depends(get_request_context),
    engine: Engine = Depends(get_engine),
    settings: Settings = Depends(get_settings),
):
    ...
    if not metrc.configured:
        raise HTTPException(409, "Connect and validate Metrc for this production facility before building a provider-ready package action.")
    try:
        proposal = RegulatoryActionProposalService(engine).package_finish_proposal(
            organization_id=context.organization_id,
            facility_id=context.facility_id,
        ...
        return _item(proposal)
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc


@router.post("/regulatory-actions/{proposal_id}/submit")
def submit_regulatory_action(
    proposal_id: str,
    context: RequestContext = Depends(get_request_context),
    engine: Engine = Depends(get_engine),
    settings: Settings = Depends(get_settings),
    ...
    if proposal is None or proposal.action_type != "prepare_regulatory_action":
        raise HTTPException(404, "Regulatory action proposal was not found in the active facility.")
    if proposal.status not in {"approved", "executed"}:
        raise HTTPException(422, "An authorized employee must approve the regulatory preview before submission.")
    proposal_payload = json.loads(proposal.payload_json or "{}")
    operation_type = str(proposal_payload.get("operation_type") or "").casefold()
    if operation_type != "package_finish":
        raise HTTPException(409, "This regulatory action does not yet have a promoted submit workflow.")
    ...
    if not metrc.configured:
        raise HTTPException(409, "A trusted production-facility Metrc mapping is required before submission.")
    if str(proposal_payload.get("jurisdiction_code") or "").upper() != metrc.state.upper():
        raise HTTPException(409, "The proposal jurisdiction no longer matches the trusted facility mapping. Regenerate it.")
    if str(proposal_payload.get("environment") or "").casefold() != metrc.environment:
        raise HTTPException(409, "The proposal environment no longer matches the trusted facility mapping. Regenerate it.")
    if str(proposal_payload.get("license_number") or "") != metrc.license_number:
        raise HTTPException(409, "The proposal license no longer matches the trusted facility mapping. Regenerate it.")
    ...
    if not transaction_id:
        raise HTTPException(409, "The approved action did not produce a traceability transaction.")
    ...
    if transaction.status != "queued":
        raise HTTPException(409, f"Regulatory transaction is {transaction.status}; reconcile it before another submission attempt.")
    try:
    ...
    return {
        ...
        "environment": metrc.environment,
        "dispatch": dispatch,
        "message": "The approved package action was submitted through the controlled regulatory engine. Provider acceptance is not the same as post-write verification.",
    }
...
def manifest_pdf(
    ...
    try:
        ...
        raise HTTPException(409, str(exc)) from exc
    safe_name = "".join(
        character for character in manifest_number if character.isalnum() or character in {"-", "_"}
    ) or "metrc-manifest"
    return Response(
        content=content,
