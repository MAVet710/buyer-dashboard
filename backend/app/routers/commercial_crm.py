...
from modules.commercial.crm_contracts import AccountInput, OpportunityInput, ActivityInput, QuoteInput
...
def writer(context: RequestContext = Depends(get_commercial_context)):
...
def products(search: str = Query('', max_length=120), crm=Depends(service)):
    with Session(crm.engine) as s:
        return [dict(id=p.id, name=p.name, sku=p.sku, base_unit=p.base_unit, retail_price=p.retail_price)
