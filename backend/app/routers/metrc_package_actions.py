...
class PackageActionRequest(BaseModel):
    ...
    reason: str = Field(default="Package 360 controlled Metrc action", min_length=3, max_length=255)
...
def package_status(
    ...
    return {
        ...
        "execution_boundary": "Every write requires exact Product-to-Item and Inventory Lot-to-Package identities, fresh provider equality, human confirmation, durable ledger evidence, and semantic readback." if ready else "Local package workflows remain available; provider writes stay disabled.",
    }
...
def active_items(
    ...
    return {
        "items": [
            {"provider_id": str(row.get("provider_id") or ""), "name": str(row.get("name") or ""), "last_modified": str(row.get("last_modified") or "")}
        ...
        "items": [
            {
                ...
                "unit_of_measure": str(row.get("unit_of_measure") or ""),
...
def preview_package_action(
    ...
    try:
        ...
        return {
            ...
            "message": "Review the business values before confirming. Any local or METRC state change invalidates this confirmation.",
        }
