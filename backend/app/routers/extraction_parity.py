...
def overview(context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    repo = _repo(engine); runs = repo.list_runs(context.organization_id, context.facility_id, include_closed=True)
    rows=[]; finished=0.0; yields=[]; qa_holds=0; toll_jobs=0; total_cogs=0.0
    for run in runs:
        mass=repo.mass_balance(context.organization_id,context.facility_id,run.id); cogs=repo.cogs_summary(context.organization_id,context.facility_id,run.id); toll=repo.get_toll_job(context.organization_id,context.facility_id,run.id); qa=repo.list_qa_events(context.organization_id,context.facility_id,run.id); trace=repo.list_traceability_transactions(context.organization_id,context.facility_id,run.id)
        finished += float(mass.get("recorded_output",0)); yields.append(float(mass.get("yield_pct",0))); total_cogs += float(cogs.get("total",0)); qa_hold = run.status == "hold" or any(event.result == "failed" for event in qa); qa_holds += int(qa_hold); toll_jobs += int(toll is not None or run.toll_processing)
        rows.append({"id":run.id,"batch_id_internal":run.batch_number,"method":run.method,"product_type":run.product_family,"strain":run.strain,"operator":run.operator,"status":run.status,"release_status":run.release_status,"toll_processing":run.toll_processing,"started_at":run.started_at,"completed_at":run.completed_at,"input_weight_g":mass.get("consumed_input",0),"finished_output_g":mass.get("recorded_output",0),"residual_loss_g":mass.get("unaccounted_balance",0),"yield_pct":mass.get("yield_pct",0),"cogs_usd":cogs.get("total",0),"cost_per_output_unit":cogs.get("cost_per_output_unit",0),"qa_hold":qa_hold,"coa_status":"failed" if any(event.result=="failed" for event in qa) else "passed" if any(event.result=="passed" for event in qa) else "pending","traceability_count":len(trace),"toll_job":_toll(toll)})
    alerts=[]
    low=sum(1 for row in rows if row["input_weight_g"]>0 and row["yield_pct"]<12)
    if low: alerts.append(f"Low yield runs: {low} below 12% yield.")
    if qa_holds: alerts.append(f"QA holds active: {qa_holds} run(s).")
    pending=sum(1 for row in rows if row["coa_status"] in {"pending","failed"})
    if pending: alerts.append(f"COA risk: {pending} run(s) pending/failed.")
...
def create_toll_job(payload: TollJobCreate, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    workflow=default_workflow_for_method(payload.method); repo=_repo(engine)
...
def compliance(run_id: str, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    repo=_repo(engine)
    try:
        run=repo.get_run(context.organization_id,context.facility_id,run_id); outputs=repo.list_outputs(context.organization_id,context.facility_id,run_id); qa=repo.list_qa_events(context.organization_id,context.facility_id,run_id); trace=repo.list_traceability_transactions(context.organization_id,context.facility_id,run_id); toll=repo.get_toll_job(context.organization_id,context.facility_id,run_id)
    except ValueError as exc:
        raise HTTPException(404,str(exc)) from exc
    return {"run":{"id":run.id,"batch_number":run.batch_number,"method":run.method,"status":run.status,"release_status":run.release_status,"license_number":run.license_number,"compliance_provider":run.compliance_provider,"toll_processing":run.toll_processing},"outputs":[{"id":row.id,"output_label":row.output_label,"compliance_package_id":row.compliance_package_id,"coa_status":row.coa_status,"status":row.status} for row in outputs],"qa_events":[{"event_type":row.event_type,"result":row.result,"coa_reference":row.coa_reference,"deviation_code":row.deviation_code,"notes":row.notes,"occurred_at":row.occurred_at} for row in qa],"traceability":[{"id":row.id,"entity_type":row.entity_type,"entity_id":row.entity_id,"status":row.status,"action":row.action,"requested_at":row.requested_at} for row in trace],"toll_job":_toll(toll)}
