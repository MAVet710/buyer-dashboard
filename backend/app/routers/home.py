...
from modules.data_hub_core import RETAIL_DATASETS
...
# Data Hub adds these durable/non-upload status rows to the retail dataset catalog.
_DATA_HUB_DURABLE_STATUS_ROWS = 6
...
def summary(context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    with Session(engine) as session:
        facility_lots = select(InventoryLot.id).where(
        ...
        inventory_quantity = float(
            session.scalar(
                select(func.coalesce(func.sum(InventoryTransaction.quantity_delta), 0.0)).where(
                    InventoryTransaction.lot_id.in_(facility_lots)
                )
            )
            or 0
        )
        ...
        plant_count = int(
            session.scalar(
                select(func.count()).select_from(CultivationPlant).where(
                    CultivationPlant.organization_id == context.organization_id,
                    CultivationPlant.facility_id == context.facility_id,
                    CultivationPlant.phase.not_in(("harvested", "destroyed")),
                )
            )
            or 0
        )
        ...
        open_orders = int(
            session.scalar(
                select(func.count()).select_from(CommercialOrder).where(
        ...
        compliance_exceptions = int(
            session.scalar(
                select(func.count()).select_from(TraceabilityTransaction).where(
        ...
        active_data_sources = int(
            session.scalar(
                select(func.count()).select_from(DataHubImport).where(
        ...
        balances = {str(product_id): float(balance or 0) for product_id, balance in balance_rows if product_id}
        ...
        sale_rows = session.execute(
            ...
            .where(
                ...
                RetailSale.sold_at >= now - timedelta(days=30),
                RetailSale.sold_at <= now,
            )
            .group_by(RetailSale.product_id)
        ).all()
        low_stock = 0
        for product_id, quantity in sale_rows:
            sold = float(quantity or 0)
            if not product_id or sold <= 0:
                continue
            daily_velocity = sold / 30.0
            days_on_hand = balances.get(str(product_id), 0.0) / daily_velocity if daily_velocity > 0 else float("inf")
            if days_on_hand <= 7:
                low_stock += 1

        open_purchase_order_ids = list(
            session.scalars(
                select(CommercialOrder.id).where(
                    CommercialOrder.organization_id == context.organization_id,
                    CommercialOrder.facility_id == context.facility_id,
        ...
        open_purchase_orders = len(open_purchase_order_ids)
        open_purchase_order_value = 0.0
        if open_purchase_order_ids:
            open_purchase_order_value = float(
                session.scalar(
                    select(
                        func.coalesce(
                            func.sum(CommercialOrderLine.quantity * CommercialOrderLine.unit_price),
                            0.0,
                        )
                    ).where(CommercialOrderLine.commercial_order_id.in_(open_purchase_order_ids))
                )
                or 0
            )
    ...
    return {
        ...
        "data_sources_total": len(RETAIL_DATASETS) + _DATA_HUB_DURABLE_STATUS_ROWS,
        "low_stock": low_stock,
        "open_purchase_orders": open_purchase_orders,
        "open_purchase_order_value": open_purchase_order_value,
    }
...
def universal_search(
    ...
    with Session(engine) as session:
        for row in session.scalars(
            ...
            results.append({"kind": "product", "id": row.id, "title": row.name, "subtitle": row.sku, "workspace": "Inventory"})
        for row in session.scalars(
            select(InventoryLot)
            .where(
                InventoryLot.organization_id == context.organization_id,
                InventoryLot.facility_id == context.facility_id,
                or_(InventoryLot.lot_code.ilike(term), InventoryLot.compliance_package_id.ilike(term)),
            )
            .limit(10)
        ):
            results.append({"kind": "package", "id": row.id, "title": row.compliance_package_id or row.lot_code, "subtitle": row.location_code, "workspace": "Inventory"})
        for row in session.scalars(
            select(CultivationPlant)
            .where(
                CultivationPlant.organization_id == context.organization_id,
                CultivationPlant.facility_id == context.facility_id,
                or_(CultivationPlant.plant_tag.ilike(term), CultivationPlant.strain_name.ilike(term)),
            )
            .limit(10)
        ):
            results.append({"kind": "plant", "id": row.id, "title": row.plant_tag, "subtitle": f"{row.strain_name} · {row.phase}", "workspace": "Inventory"})
        for row in session.scalars(
            select(ProductionOrder)
            .where(
                ProductionOrder.organization_id == context.organization_id,
                ProductionOrder.facility_id == context.facility_id,
                or_(ProductionOrder.order_number.ilike(term), ProductionOrder.product_name.ilike(term)),
            )
            .limit(10)
        ):
            results.append({"kind": "production order", "id": row.id, "title": row.order_number, "subtitle": row.product_name, "workspace": "Production"})
        for row in session.scalars(
            select(CommercialOrder)
            .where(
                CommercialOrder.organization_id == context.organization_id,
                CommercialOrder.facility_id == context.facility_id,
                CommercialOrder.order_number.ilike(term),
            )
            .limit(10)
        ):
            results.append({"kind": "commercial order", "id": row.id, "title": row.order_number, "subtitle": row.status, "workspace": "Orders"})
        for row in session.scalars(
            select(TradePartner)
            .where(TradePartner.organization_id == context.organization_id, TradePartner.name.ilike(term))
            .limit(10)
        ):
            results.append({"kind": "partner", "id": row.id, "title": row.name, "subtitle": row.partner_type, "workspace": "Orders"})
    return results[:30]
        ...
        held = list(
            session.execute(
                ...
                .where(
                    InventoryLot.organization_id == context.organization_id,
                    InventoryLot.facility_id == context.facility_id,
                    func.lower(InventoryLot.status).in_(["hold", "quarantine", "failed"]),
                )
                .limit(20)
            )
        )
        ...
        unmapped = int(
            session.scalar(
                select(func.count()).select_from(RetailSale).where(
                    RetailSale.organization_id == context.organization_id,
                    RetailSale.facility_id == context.facility_id,
                    RetailSale.product_id.is_(None),
                    RetailSale.sold_at >= now - timedelta(days=7),
                )
            )
            or 0
        )
        ...
        failed_integrations = list(
            session.scalars(
                select(IntegrationConfiguration).where(
                    IntegrationConfiguration.status == "failed",
                    ((IntegrationConfiguration.scope_type == "user") & (IntegrationConfiguration.scope_key == context.user_id))
                    | ((IntegrationConfiguration.organization_id == context.organization_id) & (IntegrationConfiguration.facility_id == context.facility_id)),
                )
            )
        )
...
def product_360(product_id: str, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    try:
        snapshot = ProductMasterRepository(engine).snapshot(context.organization_id, product_id, history_limit=20)
    ...
    with Session(engine) as session:
        balance_rows = list(
            session.execute(
                select(InventoryLot, func.coalesce(func.sum(InventoryTransaction.quantity_delta), 0.0))
                .outerjoin(InventoryTransaction, InventoryTransaction.lot_id == InventoryLot.id)
                .where(
                    InventoryLot.organization_id == context.organization_id,
                    InventoryLot.facility_id == context.facility_id,
        ...
        sales = session.execute(
            select(func.coalesce(func.sum(RetailSale.quantity), 0.0), func.coalesce(func.sum(RetailSale.net_sales), 0.0)).where(
                RetailSale.organization_id == context.organization_id,
                RetailSale.facility_id == context.facility_id,
                RetailSale.product_id == product_id,
                RetailSale.sold_at >= since,
                RetailSale.sold_at <= utc_now(),
            )
        ).one()
        order_lines = list(
            session.execute(
        ...
        production = list(
            session.scalars(
                ...
                .where(
                    ProductionOrder.organization_id == context.organization_id,
                    ProductionOrder.facility_id == context.facility_id,
                    or_(ProductionOrder.sku == product.sku, ProductionOrder.product_name == product.name),
                )
    ...
    return {
        "product": {key: getattr(product, key) for key in ("id", "sku", "name", "item_type", "base_unit", "unit_cost", "retail_price", "upc", "active")},
        ...
        "inventory": {"packages": packages, "on_hand": sum(row["balance"] for row in packages), "package_count": len(packages)},
        "sales_30d": {"quantity": float(sales[0]), "net_sales": float(sales[1]), "daily_velocity": float(sales[0]) / 30},
        "open_orders": [{"id": order.id, "order_number": order.order_number, "order_type": order.order_type, "status": order.status, "quantity": float(line.quantity), "fulfilled_quantity": float(line.fulfilled_quantity), "unit": line.unit} for line, order in order_lines],
        "production_orders": [{key: getattr(row, key) for key in ("id", "order_number", "status", "requested_units", "due_at", "product_format")} for row in production],
        "aliases": [{"alias": row.alias, "source": row.source} for row in snapshot["aliases"]],
        "mappings": [{"system_name": row.system_name, "external_id": row.external_id, "external_name": row.external_name} for row in snapshot["mappings"]],
        "value_history": [{"value_type": row.value_type, "amount": row.amount, "currency": row.currency, "effective_at": row.effective_at} for row in snapshot["value_history"]],
    }
