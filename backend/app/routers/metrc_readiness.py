...
class HarvestAddPlants(HarvestWeights):
    harvest_date: date
...
class HarvestDiscontinue(BaseModel):
    provider_confirmed: bool = False
    reason: str = Field(min_length=1, max_length=255)
...
class ManicureAddPlants(BaseModel):
    plant_weights: list[ManicurePlantWeight] = Field(min_length=1, max_length=5000)
    provider_confirmed: bool = False
...
class WholePackageTransferCreate(BaseModel):
    ...
    actual_departure_confirmed: bool = False
...
def readiness_status(
    ...
    result = MetrcProcessComplianceService(engine).readiness_summary(context.organization_id, context.facility_id, environment=environment)
    return {
        **result,
...
def sync_tags(
    ...
    return MetrcProcessComplianceService(engine).sync_available_tags(
...
def tags(
    ...
    status: str = Query(default="available", pattern="^(|available|unavailable|reserved|used|voided)$"),
    environment: str = Query(default="sandbox", pattern="^(sandbox|production)$"),
    context: RequestContext = Depends(get_request_context),
    engine: Engine = Depends(get_engine),
):
    _cultivation(context, engine)
    return {"items": MetrcProcessComplianceService(engine).list_tags(
...
def create_immature_group(payload: ImmatureGroupCreate, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    try:
        return MetrcProcessComplianceService(engine).create_immature_group(
...
def assign_vegetative_tags(
    ...
    if not metrc.configured:
        raise HTTPException(409, metrc.message)
    try:
        return MetrcProcessComplianceService(engine).assign_vegetative_tags(
...
def replace_tag(
    ...
    if not metrc.configured:
        raise HTTPException(409, metrc.message)
    try:
        return MetrcProcessComplianceService(engine).replace_plant_tag(
...
def correct_strain(plant_id: str, payload: StrainCorrection, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).correct_plant_strain(
...
@router.post("/cultivation/plants/{plant_id}/destroy")
def destroy_plant(plant_id: str, payload: PlantDestroy, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).destroy_plant(
            context.organization_id, context.facility_id, plant_id, actor=context.user_id, **payload.model_dump()
        )
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc
...
def harvest_wet_weights(harvest_id: str, payload: HarvestWeights, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).record_harvest_wet_weights(
...
def harvest_add_plants(harvest_id: str, payload: HarvestAddPlants, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).add_plants_to_harvest(
            context.organization_id, context.facility_id, harvest_id,
            plant_weights=[row.model_dump() for row in payload.plant_weights],
            harvest_date=payload.harvest_date, actor=context.user_id, provider_confirmed=payload.provider_confirmed,
        )
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc
...
def harvest_closeout(harvest_id: str, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).harvest_closeout_preview(context.organization_id, context.facility_id, harvest_id)
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc


@router.post("/cultivation/harvests/{harvest_id}/finish")
def harvest_finish(harvest_id: str, payload: HarvestFinish, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).finish_harvest(
            context.organization_id, context.facility_id, harvest_id,
            actor=context.user_id, provider_confirmed=payload.provider_confirmed,
...
@router.post("/cultivation/harvests/{harvest_id}/discontinue")
def harvest_discontinue(harvest_id: str, payload: HarvestDiscontinue, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).discontinue_harvest(
            context.organization_id, context.facility_id, harvest_id,
            actor=context.user_id, provider_confirmed=payload.provider_confirmed, reason=payload.reason,
        )
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc
...
def record_waste(payload: WasteCreate, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    try:
        ...
        return MetrcProcessComplianceService(engine).record_waste(
...
def manicure(payload: ManicureCreate, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    try:
        ...
        return MetrcProcessComplianceService(engine).create_manicure_batch(
...
@router.post("/cultivation/manicure/{batch_id}/add-plants")
def manicure_add_plants(batch_id: str, payload: ManicureAddPlants, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).add_plants_to_manicure_batch(
            context.organization_id, context.facility_id, batch_id,
            plant_weights=[row.model_dump() for row in payload.plant_weights],
            actor=context.user_id, provider_confirmed=payload.provider_confirmed,
        )
    except ValueError as exc:
...
def additive(payload: AdditiveCreate, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).record_additive(
            context.organization_id, context.facility_id, actor=context.user_id, **payload.model_dump()
        )
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc
...
def test_sample(
    ...
    if not metrc.configured:
        raise HTTPException(409, metrc.message)
    try:
        return MetrcProcessComplianceService(engine).create_test_sample(
...
def confirm_test_sample(sample_id: str, payload: TestSampleConfirm, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _cultivation(context, engine)
    try:
        return MetrcProcessComplianceService(engine).confirm_test_sample(
...
@router.get("/packages/{lot_id}/discontinue-preflight")
def package_discontinue_preflight(lot_id: str, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _inventory(context, engine)
    try:
        return MetrcProcessComplianceService(engine).package_discontinue_preflight(context.organization_id, context.facility_id, lot_id)
    except ValueError as exc:
...
def dispatch_whole_packages(payload: WholePackageTransferCreate, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    if not payload.state_transfer_confirmed:
        raise HTTPException(422, "Confirm the Metrc manifest/transfer before physical dispatch.")
    if not payload.actual_departure_confirmed:
        raise HTTPException(422, "Strict Metrc inventory transfer-out posts only at actual departure. Confirm the vehicle/package custody has left the source facility.")
    try:
        return MetrcStrictTransferService(engine).dispatch_whole_packages(
...
def departure(transfer_id: str, payload: TransferDeparture, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    try:
        return MetrcStrictTransferService(engine).confirm_departure(
...
def receive_whole_package(transfer_id: str, line_id: str, payload: TransferReceive, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    try:
        return MetrcStrictTransferService(engine).receive_whole_package(
...
def reject_package(transfer_id: str, line_id: str, payload: TransferReject, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    ...
    try:
        return MetrcStrictTransferService(engine).reject_package(
            context.organization_id, context.facility_id, transfer_id, line_id,
...
def receive_return(transfer_id: str, line_id: str, payload: TransferReturn, context: RequestContext = Depends(get_request_context), engine: Engine = Depends(get_engine)):
    _write(context); _inventory(context, engine)
    try:
        return MetrcStrictTransferService(engine).receive_rejected_return(
            context.organization_id, context.facility_id, transfer_id, line_id,
