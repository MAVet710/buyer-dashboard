...
class BomStandardUpdate(BaseModel):
    standard_labor_hours: float = 0.0
    standard_machine_hours: float = 0.0
    standard_cycle_hours: float = 0.0
...
def order_360(
    ...
    return {
        ...
        if bom is None
            ...
            key: getattr(bom, key)
            for key in ("id", "version", "output_quantity", "expected_loss_pct", "active")
        },
        "standard": _standard_payload(snapshot["standard"]),
        "variance": snapshot["variance"],
        "requirements": snapshot["requirements"],
        "reservations": [
            {key: getattr(row, key) for key in ("id", "lot_id", "quantity", "unit", "status")}
        ...
        "qa_events": [
            {
                key: getattr(row, key)
                for key in ("id", "event_type", "result", "notes", "actor", "occurred_at")
            }
            for row in snapshot["qa_events"]
        ],
        "cogs": snapshot["cogs"],
        "planned_output": snapshot["planned_output"],
...
def save_standard(
    order_id: str,
    payload: BomStandardUpdate,
    context: RequestContext = Depends(get_request_context),
    engine: Engine = Depends(get_engine),
):
    try:
        row = _service(engine).upsert_bom_standard(
...
def reserve(
    ...
    try:
        return _service(engine).reserve_bom_materials(
            organization_id=context.organization_id,
            facility_id=context.facility_id,
            order_id=order_id,
            actor=context.user_id,
...
def record_event(
    ...
    try:
        event = _service(engine).record_event(
            organization_id=context.organization_id,
            facility_id=context.facility_id,
            order_id=order_id,
            actor=context.user_id,
...
def add_output(
    ...
    try:
        row = _service(engine).add_output(
            organization_id=context.organization_id,
            facility_id=context.facility_id,
            order_id=order_id,
            actor=context.user_id,
...
def record_output(
    output_id: str,
    payload: OutputActual,
    context: RequestContext = Depends(get_request_context),
    engine: Engine = Depends(get_engine),
):
    try:
        row = _service(engine).record_output_actual(
            organization_id=context.organization_id,
            facility_id=context.facility_id,
            output_id=output_id,
            actor=context.user_id,
