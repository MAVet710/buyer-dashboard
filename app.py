...
sales_file = st.sidebar.file_uploader("Product Sales Report XLSX", type=["xlsx"])
...
if inv_file and sales_file:
    try:
        ...
        sales_raw = pd.read_excel(sales_file, header=0)
        ...
        rename_sales = {col: column_map_sales[col] for col in column_map_sales if col in sales_raw.columns}
        sales_raw = sales_raw.rename(columns=rename_sales)
        ...
        sales_raw = sales_raw.dropna(subset=["MasterCategory"])
        sales_raw["OrderDate"] = pd.to_datetime(sales_raw.get("OrderDate", datetime.now()), errors="coerce")
        sales_raw["MasterCategory"] = sales_raw["MasterCategory"].str.lower()
        sales_raw = sales_raw[~sales_raw["MasterCategory"].str.contains("accessor")]
        sales_raw = sales_raw[sales_raw["MasterCategory"] != "all"]

        date_range = sales_raw["OrderDate"].dropna().sort_values().unique()
        if len(date_range) == 0:
            st.error("No valid order dates in sales data.")
            st.stop()
        ...
        if date_start <= date_end:
            mask = (sales_raw["OrderDate"] >= date_start) & (sales_raw["OrderDate"] <= date_end)
            filtered_sales = sales_raw[mask]
