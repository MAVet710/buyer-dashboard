...
if not _EXTRACTION_PARTNER_INTEL_AVAILABLE:
    ...
    def load_partner_file(uploaded_file) -> pd.DataFrame:
        ...
        def _row_is_partner_header(values: list[str]) -> bool:
            ...
            has_required = len(normalized.intersection(required_any)) >= 4
            has_output_cluster = any(n.startswith("new metrc") for n in normalized) or any(n.startswith("new item") for n in normalized)
        ...
        def _extract_partner_blocks(preview_df: pd.DataFrame) -> tuple[pd.DataFrame, list[int]]:
            ...
            for h_i, start_idx in enumerate(header_rows):
                end_idx = header_rows[h_i + 1] if h_i + 1 < len(header_rows) else len(preview_df)
            ...
            return pd.concat(blocks, ignore_index=True, sort=False), header_rows
        ...
        def _detect_header_row(preview_df: pd.DataFrame) -> int:
            ...
            for idx in range(min(len(preview_df), 60)):
                ...
                partner_prefix_hits = sum(
                    1 for n in normalized if any(n.startswith(prefix) for prefix in partner_prefix_tokens)
                )
    ...
    def _partner_to_numeric_loose(series_or_value) -> pd.Series:
    ...
    def _partner_to_date_string(series_or_value) -> pd.Series:
        series = series_or_value if isinstance(series_or_value, pd.Series) else pd.Series(series_or_value)
        cleaned = (
            series.fillna("")
            .astype(str)
    ...
    def _partner_clean_tag_series(series_or_value) -> pd.Series:
        series = series_or_value if isinstance(series_or_value, pd.Series) else pd.Series(series_or_value)
    ...
    def _partner_sum_numeric_columns(df: pd.DataFrame, aliases: list[str], startswith: tuple[str, ...] = ()) -> pd.Series:
        ...
        return df[unique_cols].apply(_partner_to_numeric_loose).fillna(0).sum(axis=1)
    ...
    def map_partner_runs_to_ecc_shape(df: pd.DataFrame) -> pd.DataFrame:
        out = pd.DataFrame()
        out["run_date"] = _partner_to_date_string(_partner_pick(df, ["run_date", "run date", "date", "date in"]))
        ...
        out["metrc_package_id_input"] = _partner_clean_tag_series(
            _partner_pick(df, ["metrc_package_id_input", "input_package_id", "metrc"], default="")
        )
        out["metrc_package_id_output"] = _partner_clean_tag_series(_partner_pick(
            df,
        ...
        out["product_type"] = _partner_pick(df, ["product_type", "output_type", "item", "new item", "new item name 1"], default="Other")
        ...
        out["input_weight_g"] = _partner_to_numeric_loose(_partner_pick(df, ["input_weight_g", "input_weight", "input_g", "g"], default=0)).fillna(0)
        out["intermediate_output_g"] = _partner_to_numeric_loose(_partner_pick(df, ["intermediate_output_g", "intermediate_g"], default=0)).fillna(0)
        explicit_finished = _partner_to_numeric_loose(_partner_pick(df, ["finished_output_g", "finished_output", "output_g"], default=0)).fillna(0)
        ...
        out["residual_loss_g"] = _partner_to_numeric_loose(_partner_pick(df, ["residual_loss_g", "residual_g", "waste_g", "waste"], default=0)).fillna(0)
        out["yield_pct"] = _partner_to_numeric_loose(_partner_pick(df, ["yield_pct", "yield", "yield %", "yield (%)"], default=0)).fillna(0)
        out["post_process_efficiency_pct"] = _partner_to_numeric_loose(_partner_pick(df, ["post_process_efficiency_pct", "post_efficiency_pct", "efficiency"], default=0)).fillna(0)
        out["operator"] = _partner_pick_series(df, ["operator", "user"], default="").fillna("").astype(str).str.strip()
        out["machine_line"] = _partner_pick(df, ["machine_line", "line", "location", "location 1"], default="")
        ...
        base_notes = _partner_pick_series(df, ["notes"], default="").fillna("").astype(str)
        run_hours = _partner_pick_series(df, ["time (hours)", "time_hours", "run_hours"], default="")
...
def render_extraction_command_center():
    ...
    with inputs_tab:
        ...
        if "ecc_pending_mapped_upload" not in st.session_state:
            ...
            st.session_state.ecc_pending_detected_header_row = 0
            st.session_state.ecc_pending_detected_header_rows = []
            st.session_state.ecc_pending_source_columns = []
        if uploaded is not None:
            try:
                ...
                detected_header_row = int(uploaded_df.attrs.get("detected_header_row", 0))
                if mapped_df is None or mapped_df.empty:
                    ...
                    st.session_state.ecc_pending_detected_header_row = detected_header_row
                    st.session_state.ecc_pending_detected_header_rows = [
                        int(x) for x in uploaded_df.attrs.get("detected_header_rows", [detected_header_row])
                    ]
                    st.session_state.ecc_pending_source_columns = [str(c) for c in uploaded_df.columns]
        ...
        pending_header_row = int(st.session_state.get("ecc_pending_detected_header_row", 0))
        pending_header_rows = st.session_state.get("ecc_pending_detected_header_rows", [pending_header_row])
        pending_source_columns = st.session_state.get("ecc_pending_source_columns", [])
        has_pending_upload = isinstance(pending_df, pd.DataFrame) and not pending_df.empty and bool(pending_hash)
        if has_pending_upload:
            ...
            st.caption(f"Detected header row index: {pending_header_row} (0-indexed in raw file preview).")
            if len(pending_header_rows) > 1:
                st.caption(f"Detected repeated header rows: {pending_header_rows}")
            with st.expander("Source column check", expanded=False):
                st.write(pd.DataFrame({"source_columns": pending_source_columns}))
            if parse_warnings:
                for warning_text in parse_warnings:
                    st.caption(f"⚠️ {warning_text}")
            ...
            st.caption("No staged extraction upload yet. Upload a file first, then use push controls below.")
            c_force, c_refresh = st.columns(2)
            with c_force:
                st.button(
                    "Force Push Upload to Run Analytics",
                    key="ecc_force_push_disabled",
                    disabled=True,
                )
            with c_refresh:
                if st.button("Refresh Run Analytics Now", key="ecc_refresh_push_no_stage"):
                    st.rerun()
