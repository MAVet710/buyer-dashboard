...
def read_delivery_file(uploaded_file):
    ...
    if name.endswith((".xlsx", ".xls")):
        # header detection (similar to Dutchie exports with meta rows)
        tmp = pd.read_excel(uploaded_file, header=None)
        header_row = 0
...
def _extract_delivery_from_pdf(uploaded_file):
    ...
    try:
        ...
        if rows:
            # If first row looks like header, keep; else build generic columns
            # We will just return a raw DF and let downstream column detection handle it.
    ...
    try:
        ...
        # crude: keep lines that have a qty-like number and at least a couple words
        parsed = []
        for ln in lines:
...
# ============================================================
    ...
    # Track uploads for God viewer (keeps your existing behavior)
    current_user = st.session_state.admin_user if st.session_state.is_admin else "trial_user"
    ...
    # Cache raw dataframes to avoid re-upload pain between sections
    if delivery_file is not None:
        try:
    ...
    # If PDF parsing fell back to a single raw column, surface that cleanly
    ...
    del_name_aliases = [
        "product", "productname", "item", "itemname", "name", "description", "sku name", "product name"
    ]
    del_qty_aliases = [
    ...
    delivery = delivery_raw.rename(
        columns={
            del_date_col: "received_date",
            del_name_col: "product_name",
            del_qty_col: "qty_received",
        }
    ).copy()
    ...
    ds_units_aliases = [
        "units", "unitssold", "units sold", "quantity", "quantitysold", "qty", "qtysold", "items sold", "itemsold"
    ]
    ...
    lookback_days = st.sidebar.slider("Days before delivery (baseline window)", 1, 30, 7)
    lookforward_days = st.sidebar.slider("Days after delivery (impact window)", 1, 30, 7)
    ...
    # show most positive deltas first
    ...
    # show what was received for the selected date(s)
    if selected_date != "All delivery dates":
