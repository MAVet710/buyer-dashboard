...
with st.sidebar:
    ...
    trial_key = st.text_input("Trial Key")

    is_admin = username == "God" and password == "Major420"
    valid_trial_keys = ["trial123", "guest420"]
    is_trial_user = trial_key in valid_trial_keys

    if is_admin:
        st.success("Admin login successful.")
    elif is_trial_user:
        st.success("Trial access granted.")
    elif username or password or trial_key:
        st.error("Invalid credentials or trial key.")

if not (is_admin or is_trial_user):
    st.warning("Please log in or use a valid trial key to access the dashboard.")
    st.stop()
...
if inv_file and product_sales_file:
    try:
        ...
        def extract_size(name):
            name = str(name).lower()
            if any(kw in name for kw in ["28g", "1oz", "1 oz"]):
                return "1oz"
