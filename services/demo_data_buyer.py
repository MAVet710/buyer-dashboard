...
# Scale changes product / row volume only. Every sandbox uses the same 120-day
# sales window so velocity, DOH, trends, Buyer Intelligence, and delivery impact
# are comparable across demo sizes.
DATASET_SCALES = {
...
_BRANDS = [
    "Harbor House", "Bay State Botanicals", "Liberty Leaf", "Commonwealth Craft",
    "Veteran Grown", "South Coast Reserve",
]
_VENDORS = [
    "Atlantic Cultivation", "Pioneer Valley Gardens", "Cape Cod Cannabis",
    "Berkshire Bloom", "Merrimack Organics",
]
_CATEGORY_TEMPLATES = {
    "flower": [
        ("Flower", "3.5g", 3.5, 38.0), ("Flower", "7g", 7.0, 66.0),
        ("Flower", "14g", 14.0, 112.0), ("Flower", "28g", 28.0, 190.0),
    ],
    "pre rolls": [("Pre-Roll", "1g", 1.0, 12.0), ("Pre-Roll Pack", "2.5g", 2.5, 29.0)],
    "vapes": [
        ("Live Resin Vape", "0.5g", 0.5, 42.0), ("Distillate Vape", "1g", 1.0, 48.0),
        ("Live Rosin Vape", "0.5g", 0.5, 58.0),
    ],
    "edibles": [
        ("Gummies", "100mg", 0.0, 24.0), ("High Dose Gummies", "500mg", 0.0, 46.0),
        ("Chocolate", "100mg", 0.0, 26.0),
    ],
    "concentrates": [
        ("Live Resin", "1g", 1.0, 48.0), ("Hash Rosin", "1g", 1.0, 68.0),
        ("Badder", "1g", 1.0, 44.0),
    ],
...
def _catalog(product_count: int, rng: random.Random, profile: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for idx in range(product_count):
        ...
        rows.append({
            ...
            "retail_gross_margin_pct": round((retail_price - unit_cost) / retail_price * 100.0, 2),
            "wholesale_gross_profit": round(wholesale_price - unit_cost, 2),
            "wholesale_gross_margin_pct": round((wholesale_price - unit_cost) / wholesale_price * 100.0, 2),
            "on_hand": on_hand,
...
def _inventory(catalog: list[dict[str, Any]], today: date, problems: set[str]) -> pd.DataFrame:
    ...
    for idx, p in enumerate(catalog):
        ...
        rows.append({
            ...
            "Received Date": (today - timedelta(days=received_days)).isoformat(),
            ...
            "COA ID": p["coa_id"],
            "Lab Status": "Pending" if on_hold else "Passed",
...
def _sales(
    ...
    # Guarantee the inclusive reporting window really is 120 days rather than
    # merely sampling from a 120-day range.
    if len(frame) >= 2:
...
def _calibrate_inventory_to_sales(catalog: list[dict[str, Any]], sales: pd.DataFrame, days: int) -> None:
...
def _detail(catalog: list[dict[str, Any]], sales: pd.DataFrame, days: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    sold = sales.groupby("SKU", as_index=False).agg(
    ...
    for p in catalog:
        ...
        priority = (
            "1 – Reorder ASAP" if 0 < doh <= 7
            else "2 – Watch Closely" if doh <= 21 and daily > 0
            else "4 – Dead Item" if daily == 0
            else "3 – Comfortable Cover"
        )
        rows.append({
            "subcategory": p["category"],
            "product_name": p["product_name"],
            "strain_type": p["strain_type"],
            "packagesize": p["size_label"],
            "onhandunits": p["on_hand"],
            "unitssold": units,
            "avgunitsperday": daily,
            "daysonhand": int(round(doh)),
            "reorderqty": reorder,
    ...
    detail["reorderpriority"] = detail.apply(
        lambda r: (
            "1 – Reorder ASAP" if 0 < r["daysonhand"] <= 7
            else "2 – Watch Closely" if r["daysonhand"] <= 21 and r["avgunitsperday"] > 0
            else "4 – Dead Item" if r["avgunitsperday"] == 0
            else "3 – Comfortable Cover"
        ),
        axis=1,
    )
...
def _manifest(catalog: list[dict[str, Any]], today: date, rng: random.Random, problems: set[str]) -> pd.DataFrame:
    ...
    for p in delivered:
        ...
        rows.append({
            "Manifest #": "MAN-DEMO-2026-071",
            "Received Date": datetime.combine(row_received, time(10, 30)).isoformat(sep=" "),
            "Vendor": p["vendor"],
            ...
            "COA ID": p["coa_id"],
            "Unit Cost": p["unit_cost"],
            "Extended Cost": round(qty * p["unit_cost"], 2),
    ...
    return pd.DataFrame([
        {
            ...
            "topic": topic,
            "answer": answer,
            "source_citation": f"SYNTHETIC-DEMO-{idx + 1:02d}",
            "source_url": "https://example.invalid/demo",
            "last_updated": today.isoformat(),
            "review_status": "demo-only",
        }
...
def _white_label(catalog: list[dict[str, Any]], today: date, profile: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "wl_strain_name": flower["strain"],
        "wl_strain_type": flower["strain_type"],
        "wl_cultivator_name": flower["vendor"],
        "wl_vendor_name": flower["vendor"],
        "wl_bulk_weight_value": 10.0,
        "wl_bulk_weight_unit": "lb",
        "wl_bulk_total_cost_usd": 7200.0,
        "wl_coa_link": "https://example.invalid/demo-coa.pdf",
        "wl_thca_pct": 26.4,
        "wl_total_thc_pct": 28.1,
        "wl_terpene_pct": 2.7,
        "wl_cultivator_license_number": profile["license_number"],
        "wl_source_metrc_package_id": flower["package_id"],
        "wl_batch_or_lot_number": flower["batch"],
        "wl_harvest_date": today - timedelta(days=45),
        "wl_testing_date": today - timedelta(days=28),
        "wl_coa_status": "Passed",
        "wl_label_review_status": "Ready",
        "wl_trim_loss_pct": 4.5,
        "wl_labor_cost_total_usd": 1650.0,
        "wl_other_costs_usd": 420.0,
        "wl_compliance_admin_cost_usd": 275.0,
        "white_label_active_scenario_name": "Demo Launch Mix",
        "white_label_package_plan": plan,
        "white_label_saved_scenarios": {"Demo Launch Mix": {"white_label_package_plan": plan}},
    }


def build_buyer_demo(
    today: date,
    *,
    scale: str = "medium",
    seed: int = 710,
    company_seed: int | None = None,
    ...
    return {
        ...
        "compliance": _compliance(today),
        "detail": detail,
        "detail_product": detail_product,
        "white_label": _white_label(catalog, today, profile),
        "budget": budget,
        "reporting_days": cfg["days"],
    }
