...
def _score(source: str, aliases: Sequence[str]) -> float:
    source_norm = _norm(source); source_tokens = set(source_norm.split()); best = 0.0
    for alias in aliases:
        alias_norm = _norm(alias)
        if not alias_norm: continue
        if source_norm == alias_norm: return 1.0
...
def _validate_proposals(proposals: Sequence[Mapping[str, Any]], *, columns: Sequence[str], requirements: Mapping[str, Sequence[str]], already_mapped: Mapping[str, str]) -> list[dict[str, Any]]:
    allowed_columns = {str(column): str(column) for column in columns}; allowed_fields = set(requirements); used_columns = {str(value) for value in already_mapped.values() if value}; output = []; seen_fields = set()
    for raw in proposals:
        field = str(raw.get("required_field") or raw.get("field") or "").strip(); source = str(raw.get("source_column") or raw.get("column") or "").strip()
        if field not in allowed_fields or source not in allowed_columns or field in already_mapped or field in seen_fields or source in used_columns: continue
        try: confidence = float(raw.get("confidence", 0.0))
        ...
        used_columns.add(source); seen_fields.add(field)
    return output
...
def _heuristic_proposals(columns: Sequence[str], requirements: Mapping[str, Sequence[str]], already_mapped: Mapping[str, str]) -> list[dict[str, Any]]:
    used = {str(value) for value in already_mapped.values() if value}; proposals = []
    for field, aliases in requirements.items():
        if field in already_mapped: continue
        ...
        proposals.append({"required_field": field, "source_column": str(source), "confidence": round(score, 2), "reason": "Header similarity to the required field/known aliases."}); used.add(str(source))
    return proposals
...
def suggest_column_mapping(columns: Sequence[str], requirements: Mapping[str, Sequence[str]], *, existing_matches: Mapping[str, str] | None = None, dataset_label: str = "uploaded dataset", api_key: str | None = None, model: str | None = None, engine: Engine | None = None, organization_id: str = "", facility_id: str = "", source_vendor: str = "") -> dict[str, Any]:
    ...
    if engine is not None and organization_id and facility_id:
        memory_matches = MappingMemory(engine).approved(organization_id=organization_id, facility_id=facility_id, dataset_type=dataset_label, source_vendor=source_vendor or dataset_label, columns=clean_columns)
        for key, value in memory_matches.items(): matches.setdefault(key, value)
    ...
    if still_unresolved:
        ...
        router = _env_router()
        if api_key:
            router = ProviderRouter({"gemini": GeminiProvider(api_key=api_key, model=model or os.getenv("GEMINI_MODEL", "gemini-2.5-flash"))}, order=["gemini"], allow_cloud_fallback=True)
        try:
            ...
            parsed = parse_structured(decision.response) or {}; raw = parsed.get("mappings") or parsed.get("proposals") or []
    ...
    for proposal in ai_proposals: merged[proposal["required_field"]] = proposal
    for proposal in heuristic: merged.setdefault(proposal["required_field"], proposal)
    proposals = list(merged.values()); mapped_fields = set(matches) | {proposal["required_field"] for proposal in proposals}
