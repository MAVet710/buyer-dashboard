"""QuickBooks Online OAuth and company-health helpers.
...
def refresh_quickbooks_token(
    *,
    ...
    try:
        ...
        raise QuickBooksError(f"QuickBooks token refresh failed: {exc}") from exc
    if response.status_code in {400, 401, 403}:
        raise QuickBooksError("QuickBooks rejected the saved OAuth credentials or refresh token.")
    if response.status_code == 429:
        raise QuickBooksError("QuickBooks rate limited the token refresh.")
    if not response.ok:
        raise QuickBooksError(f"QuickBooks token refresh failed with HTTP {response.status_code}.")
    try:
        payload = response.json()
    except ValueError as exc:
        raise QuickBooksError("QuickBooks returned a non-JSON OAuth response.") from exc
...
def fetch_quickbooks_company_info(
    *,
    ...
    try:
        ...
        raise QuickBooksError(f"QuickBooks company check failed: {exc}") from exc
    if response.status_code in {401, 403}:
        raise QuickBooksError("QuickBooks access token is not authorized for the configured company.")
    if response.status_code == 429:
        raise QuickBooksError("QuickBooks rate limited the company check.")
    if not response.ok:
        raise QuickBooksError(f"QuickBooks company check failed with HTTP {response.status_code}.")
    try:
        payload = response.json()
    except ValueError as exc:
        raise QuickBooksError("QuickBooks returned a non-JSON company response.") from exc
