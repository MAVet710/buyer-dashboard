...
def calculate_sales_window(sales_df: pd.DataFrame, selected_days: int):
    ...
    sales_col = _detect(frame.columns, ["net sales", "netsales", "gross sales", "grosssales", "total sales", "totalsales", "sales", "revenue"])
    date_col = _detect(frame.columns, ["order time", "order date", "date", "sold at", "soldat", "sale date", "transaction date"])
    ...
    if date_col is not None:
        ...
        if pd.notna(max_date):
            frame = frame[frame[date_col] >= max_date - pd.Timedelta(days=max(int(selected_days), 1) - 1)]
...
def calculate_active_inventory_cost(inventory_df: pd.DataFrame, cogs_pct: float, include_dead: bool, include_quarantine: bool, include_accessories: bool):
    ...
    frame.columns = frame.columns.astype(str).str.strip().str.lower()
    qty_col = _detect(frame.columns, ["available", "on hand", "onhand", "on hand total", "onhandtotal", "quantity", "qty"])
    cost_col = _detect(frame.columns, ["cost", "unit cost", "unitcost", "cogs", "wholesale price", "wholesaleprice"])
    retail_col = _detect(frame.columns, ["med price", "medprice", "retail price", "retailprice", "retail", "msrp"])
    cat_col = _detect(frame.columns, ["master category", "mastercategory", "subcategory", "category"])
    dos_col = _detect(frame.columns, ["days on hand", "daysonhand", "days of supply", "daysofsupply", "doh", "dos"])
    name_col = _detect(frame.columns, ["product name", "productname", "product", "item name", "itemname", "item", "name"])
    status_col = _detect(frame.columns, ["status", "inventory status", "inventorystatus"])
    if qty_col is None:
        return frame, 0.0
    ...
    if not include_quarantine:
        if status_col is not None:
            frame = frame[~frame[status_col].astype(str).str.lower().str.contains("quarantine|hold", na=False, regex=True)]
        elif name_col is not None:
            frame = frame[~frame[name_col].astype(str).str.lower().str.contains("quarantine", na=False)]
...
def build_budget(inventory_df: pd.DataFrame, sales_df: pd.DataFrame, *, selected_days: int = 30, target_dos: float = 45, cogs_pct: float = 0.50, safety_stock: float = 0.10, growth_adj: float = 0.0, include_dead: bool = False, include_quarantine: bool = False, include_accessories: bool = False, on_order_cost: float = 0.0):
    ...
    if inv_cat and sales_cat:
        ...
        for category in categories:
            ...
            category_rows.append({"Category": category, "Current Inventory at Cost": current, "Target Inventory at Cost": cat_target, "Recommended Budget": cat_target - current})
