...
def public_citations(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in results:
        ...
        seen.add(key)
        output.append({key: row.get(key) for key in ("title", "source", "source_type", "authority_level", "jurisdiction", "effective_date", "updated_at", "version", "url", "page_or_section", "score")})
    return output
