        ...
        *,
        ...
        with Session(self.engine) as session:
            ...
            source_payload: dict[str, Any] = {
                ...
                "Line": [
                    {
                        "Amount": round(float(payment.amount_usd), 2),
                        "LinkedTxn": [
                            {
                                "TxnId": invoice_link.external_id,
                                "TxnType": "Invoice",
                            }
                        ],
                    }
            ...
            if payment_link is not None and payment_link.payload_hash == payload_hash:
                return {
                    ...
                    "skipped": True,
                    "local_id": payment.id,
                    "qbo_id": payment_link.external_id,
                    "entity": "payment",
                    "invoice_id": invoice.id,
                    "qbo_invoice_id": invoice_link.external_id,
                }
            if payment_link is not None:
                raise QuickBooksSyncError(
                    "This recorded payment already has a QuickBooks identity but its local accounting facts changed. "
                    "Automatic Payment mutation is blocked; reconcile the existing QBO payment explicitly."
                )
