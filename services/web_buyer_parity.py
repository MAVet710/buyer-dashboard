...
def exact_buyer_intelligence(detail_product: pd.DataFrame, sales_df: pd.DataFrame, lookback_days: int) -> dict[str,Any]:
    ...
    revenue=(sales_df.groupby("product_name",dropna=False)["net_sales"].sum().rename("revenue") if "net_sales" in sales_df else pd.Series(dtype=float,name="revenue")); frame=frame.merge(revenue,on="product_name",how="left") if not revenue.empty else frame.assign(revenue=0.0); frame["revenue"]=pd.to_numeric(frame["revenue"],errors="coerce").fillna(0); frame["avg_daily_units"]=pd.to_numeric(frame["units_sold"],errors="coerce").fillna(0)/max(int(lookback_days),1); frame["risk_flag"]=np.where(pd.to_numeric(frame["days_of_cover"],errors="coerce").notna()&(pd.to_numeric(frame["days_of_cover"],errors="coerce")<=14),"Reorder Risk","Monitor")
    by_category=frame.groupby("category",as_index=False)[["units_sold","revenue"]].sum().sort_values("units_sold",ascending=False); priorities=build_assortment_priorities(frame[["product_name","category","strain_type","package_size","avg_daily_units","on_hand_units","units_sold","days_of_cover"]]); summary={"total_units_sold":float(frame["units_sold"].sum()),"total_revenue":float(frame["revenue"].sum()),"at_risk_skus":int((frame["risk_flag"]=="Reorder Risk").sum()),"tracked_skus":int(len(frame))}; return {"summary":summary,"by_category":by_category,"by_product":frame.sort_values("units_sold",ascending=False),"purchase_priorities":priorities}


def exact_trends(detail_product:pd.DataFrame,sales_df:pd.DataFrame,trend_days:int,compare_days:int,run_rate_multiplier:float,top_n:int)->dict[str,Any]:
    del compare_days
    sales=sales_df.copy(); denom=max(int(trend_days),1); mult=float(run_rate_multiplier); revenue="net_sales" if "net_sales" in sales else None
    category=sales.groupby("mastercategory",dropna=False)["unitssold"].sum().reset_index(); category["units_per_day"]=(category["unitssold"]/denom)*mult; total=float(category["unitssold"].sum()); category["unit_share"]=np.where(total>0,category["unitssold"]/total,0.0); category=category.sort_values("unitssold",ascending=False)
    package=sales.groupby("packagesize",dropna=False)["unitssold"].sum().reset_index(); package["units_per_day"]=(package["unitssold"]/denom)*mult; package=package.sort_values("unitssold",ascending=False)
    columns=["product_name","mastercategory","strain_type","packagesize","unitssold"]+([revenue] if revenue else []); sku=sales[columns].copy(); sku["units_per_day"]=(sku["unitssold"]/denom)*mult
    if revenue: sku=sku.rename(columns={revenue:"revenue"}); sku["avg_price"]=np.where(sku["unitssold"]>0,sku["revenue"]/sku["unitssold"],0.0)
    sku=sku.sort_values("units_per_day",ascending=False); sellers=sku.groupby("mastercategory",sort=True,group_keys=False).head(max(1,int(top_n))).copy()
    fast=detail_product.copy(); fast["units_per_day"]=(pd.to_numeric(fast.get("unitssold",0),errors="coerce").fillna(0)/denom)*mult; fast["risk_score"]=fast["units_per_day"]/np.maximum(pd.to_numeric(fast.get("onhandunits",0),errors="coerce").fillna(0),1); fast=fast.sort_values("risk_score",ascending=False).head(50)
    return {"category_mix":category,"package_size_mix":package,"top_movers":sku.head(50),"best_sellers_by_category":sellers,"fast_movers_low_stock":fast}
